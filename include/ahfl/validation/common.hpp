...
inline void require_non_empty(DiagnosticBag &diagnostics,
    ...
    emit_validation_error(diagnostics,
                          code::kRequiredFieldEmpty,
                          scoped_label(scope, field) + " must not be empty");
}

inline void require_optional_non_empty(
    ...
    emit_validation_error(
        diagnostics, code::kOptionalFieldEmpty, scoped_label(scope, field) + " " + std::string(suffix));
}

[[nodiscard]] inline bool package_identity_equals(
    const std::optional<handoff::PackageIdentity> &lhs,
...
[[nodiscard]] inline bool failure_summary_equals(
...
inline void validate_package_identity(const handoff::PackageIdentity &identity,
    ...
    require_version(
        diagnostics, scope, base + " format_version", identity.format_version, handoff::kFormatVersion);
...
inline void validate_failure_summary_field(
    const runtime_session::RuntimeFailureSummary &summary,
    DiagnosticBag &diagnostics,
    std::string_view scope,
...
inline void validate_failure_summary_owner(const runtime_session::RuntimeFailureSummary &summary,
    ...
    if (summary.message.empty()) {
        emit_validation_error(
            ...
            scoped_message(scope, std::string(owner_name) +
