...
bool test_parse_bool() {
    ...
    if (!t || !f) return false;
    if ((*t)->kind != Kind::Bool || (*t)->bool_val != true) return false;
    if ((*f)->kind != Kind::Bool || (*f)->bool_val != false) return false;
    return true;
}
...
bool test_parse_int() {
    ...
    if (!result) return false;
    if ((*result)->kind != Kind::Int) return false;
    if ((*result)->int_val != 42) return false;

    auto neg = parse_json("-100");
    if (!neg) return false;
    if ((*neg)->int_val != -100) return false;
    return true;
}
...
bool test_parse_float() {
    ...
    if (!result) return false;
    if ((*result)->kind != Kind::Float) return false;
    if ((*result)->float_val < 3.13 || (*result)->float_val > 3.15) return false;
    return true;
}
...
bool test_parse_string() {
    ...
    if (!result) return false;
    if ((*result)->kind != Kind::String) return false;
    if ((*result)->string_val != "hello") return false;
    return true;
}
...
bool test_parse_array() {
    ...
    if (!result) return false;
    if ((*result)->kind != Kind::Array) return false;
    if ((*result)->array_items.size() != 3) return false;
    if ((*result)->array_items[0]->int_val != 1) return false;
    if ((*result)->array_items[2]->int_val != 3) return false;
    return true;
}
...
bool test_serialize_roundtrip() {
    ...
    if (!null_v || serialize_json(**null_v) != "null") return false;

    // Bool
    auto bool_v = parse_json("true");
    if (!bool_v || serialize_json(**bool_v) != "true") return false;

    // Int
    auto int_v = parse_json("42");
    if (!int_v || serialize_json(**int_v) != "42") return false;

    // String
    auto str_v = parse_json("\"hello\"");
    if (!str_v || serialize_json(**str_v) != "\"hello\"") return false;

    // Array
    auto arr_v = parse_json("[1,2,3]");
    if (!arr_v || serialize_json(**arr_v) != "[1,2,3]") return false;
...
bool test_get_accessor() {
    ...
    if (name_field->kind != Kind::String || name_field->string_val != "Alice") return false;

    auto *age_field = obj.get("age");
    if (!age_field) return false;
    if (age_field->kind != Kind::Int || age_field->int_val != 30) return false;

    // Missing key returns nullptr
    if (obj.get("missing") != nullptr) return false;
...
bool test_as_string() {
    ...
    if (!val || *val != "world") return false;

    // Non-string returns nullopt
    auto int_v = parse_json("42");
    if (!int_v) return false;
    if ((*int_v)->as_string().has_value()) return false;
...
bool test_as_int() {
    ...
    if (!val || *val != 99) return false;

    // Non-int returns nullopt
    auto str_v = parse_json("\"hello\"");
    if (!str_v) return false;
    if ((*str_v)->as_int().has_value()) return false;
...
bool test_as_bool() {
    ...
    if (!fval || *fval != false) return false;

    // Non-bool returns nullopt
    auto int_v = parse_json("42");
    if (!int_v) return false;
    if ((*int_v)->as_bool().has_value()) return false;
...
bool test_unicode_escape() {
    ...
    if (!result) return false;
    if ((*result)->kind != Kind::String) return false;
    if ((*result)->string_val != "A") return false;

    // \u4e16 is '世'
    auto cn = parse_json("\"\\u4e16\"");
    if (!cn) return false;
    if ((*cn)->string_val.empty()) return false;
...
bool test_nested_structures() {
    ...
    if (!result) return false;
    auto &root = **result;
    if (root.kind != Kind::Object) return false;

    auto *users = root.get("users");
    if (!users || users->kind != Kind::Array) return false;
    if (users->array_items.size() != 2) return false;

    auto &first = *users->array_items[0];
    if (first.kind != Kind::Object) return false;
    auto *name = first.get("name");
    if (!name || name->string_val != "Bob") return false;
    auto *active = first.get("active");
    if (!active || active->bool_val != true) return false;
    ...
    if (!reparsed_users || reparsed_users->array_items.size() != 2) return false;
