...
ExecutionJournalValidationResult validate_execution_journal(const ExecutionJournal &journal) {
    ...
    if (journal.format_version != kExecutionJournalFormatVersion) {
        result.diagnostics.error().message("execution journal validation encountered unsupported format_version '" + journal.format_version + "'").emit();
    }

    if (journal.source_runtime_session_format_version !=
        runtime_session::kRuntimeSessionFormatVersion) {
        result.diagnostics.error().message("execution journal validation encountered unsupported source_runtime_session_format_version '" + journal.source_runtime_session_format_version + "'").emit();
    }

    if (journal.source_execution_plan_format_version != handoff::kExecutionPlanFormatVersion) {
        result.diagnostics.error().message("execution journal validation encountered unsupported source_execution_plan_format_version '" + journal.source_execution_plan_format_version + "'").emit();
    }

    if (journal.workflow_canonical_name.empty()) {
        result.diagnostics.error().message("execution journal validation contains empty workflow_canonical_name").emit();
    }

    if (journal.session_id.empty()) {
        result.diagnostics.error().message("execution journal validation contains empty session_id").emit();
    }

    if (journal.events.empty()) {
        result.diagnostics.error().message("execution journal validation contains no events").emit();
        return result;
    }

    if (journal.events.front().kind != ExecutionJournalEventKind::SessionStarted) {
        result.diagnostics.error().message("execution journal validation must begin with 'session_started'").emit();
    }
    ...
    for (const auto &event : journal.events) {
        ...
        if (event.workflow_canonical_name.empty()) {
            result.diagnostics.error().message("execution journal validation event '" + kind_name + "' contains empty workflow_canonical_name").emit();
        } else if (event.workflow_canonical_name != journal.workflow_canonical_name) {
            result.diagnostics.error().message("execution journal validation event '" + kind_name + "' workflow_canonical_name does not match journal").emit();
        }

        std::unordered_set<std::string> dependency_names;
        for (const auto &dependency : event.satisfied_dependencies) {
            if (dependency.empty()) {
                result.diagnostics.error().message("execution journal validation event '" + kind_name + "' contains empty satisfied dependency").emit();
                continue;
            }

            if (!dependency_names.insert(dependency).second) {
                result.diagnostics.error().message("execution journal validation event '" + kind_name + "' contains duplicate satisfied dependency '" + dependency + "'").emit();
            }
        }
        ...
        for (const auto &selector : event.used_mock_selectors) {
            if (selector.empty()) {
                result.diagnostics.error().message("execution journal validation event '" + kind_name + "' contains empty used mock selector").emit();
                continue;
            }

            if (!mock_selectors.insert(selector).second) {
                result.diagnostics.error().message("execution journal validation event '" + kind_name + "' contains duplicate used mock selector '" + selector + "'").emit();
            }
        }
        ...
        if (terminal_workflow_kind.has_value()) {
            result.diagnostics.error().message("execution journal validation contains events after '" + terminal_workflow_name + "'").emit();
            break;
        }
        ...
        case ExecutionJournalEventKind::SessionStarted:
            if (event.node_name.has_value()) {
                result.diagnostics.error().message("execution journal validation 'session_started' must not contain node_name").emit();
            }
            if (event.execution_index.has_value()) {
                result.diagnostics.error().message("execution journal validation 'session_started' must not contain execution_index").emit();
            }
            if (event.failure_summary.has_value()) {
                result.diagnostics.error().message("execution journal validation 'session_started' must not contain failure_summary").emit();
            }
            break;
        case ExecutionJournalEventKind::WorkflowCompleted:
        case ExecutionJournalEventKind::WorkflowFailed:
            if (event.node_name.has_value()) {
                result.diagnostics.error().message("execution journal validation '" + kind_name + "' must not contain node_name").emit();
            }
            if (event.execution_index.has_value()) {
                result.diagnostics.error().message("execution journal validation '" + kind_name + "' must not contain execution_index").emit();
            }
            if (event.kind == ExecutionJournalEventKind::WorkflowCompleted) {
                if (event.failure_summary.has_value()) {
                    result.diagnostics.error().message("execution journal validation 'workflow_completed' must not contain failure_summary").emit();
                }
            } else {
                if (!event.failure_summary.has_value()) {
                    result.diagnostics.error().message("execution journal validation 'workflow_failed' requires failure_summary").emit();
                } else if (event.failure_summary->kind !=
                           runtime_session::RuntimeFailureKind::WorkflowFailed) {
                    result.diagnostics.error().message("execution journal validation 'workflow_failed' failure_summary kind must be WorkflowFailed").emit();
                }
            }
        ...
        case ExecutionJournalEventKind::NodeFailed: {
            if (!event.node_name.has_value() || event.node_name->empty()) {
                result.diagnostics.error().message("execution journal validation event '" + kind_name + "' requires non-empty node_name").emit();
                break;
            }
            if (!event.execution_index.has_value()) {
                result.diagnostics.error().message("execution journal validation event '" + kind_name + "' requires execution_index").emit();
                break;
            }

            if (previous_execution_index.has_value() &&
                *event.execution_index < *previous_execution_index) {
                result.diagnostics.error().message("execution journal validation event '" + kind_name + "' decreases execution_index ordering").emit();
            }
            ...
            if (event.failure_summary.has_value() && event.failure_summary->node_name.has_value() &&
                *event.failure_summary->node_name != *event.node_name) {
                result.diagnostics.error().message("execution journal validation event '" + kind_name + "' failure_summary node_name does not match node_name").emit();
            }
            switch (event.kind) {
            case ExecutionJournalEventKind::NodeBecameReady:
                if (phase != NodeEventPhase::None) {
                    result.diagnostics.error().message("execution journal validation node '" + *event.node_name + "' duplicates 'node_became_ready' phase").emit();
                }
                if (event.failure_summary.has_value()) {
                    result.diagnostics.error().message("execution journal validation event '" + kind_name + "' must not contain failure_summary").emit();
                }
                phase = NodeEventPhase::BecameReady;
                break;
            case ExecutionJournalEventKind::NodeStarted:
                if (phase != NodeEventPhase::BecameReady) {
                    result.diagnostics.error().message("execution journal validation node '" + *event.node_name + "' starts before 'node_became_ready'").emit();
                }
                if (event.failure_summary.has_value()) {
                    result.diagnostics.error().message("execution journal validation event '" + kind_name + "' must not contain failure_summary").emit();
                }
            ...
            case ExecutionJournalEventKind::NodeCompleted:
                if (phase != NodeEventPhase::Started) {
                    result.diagnostics.error().message("execution journal validation node '" + *event.node_name + "' completes before 'node_started'").emit();
                }
                if (!completed_node_keys.insert(key).second) {
                    result.diagnostics.error().message("execution journal validation node '" + *event.node_name + "' duplicates 'node_completed' phase").emit();
                }
                if (event.failure_summary.has_value()) {
                    result.diagnostics.error().message("execution journal validation event '" + kind_name + "' must not contain failure_summary").emit();
                }
            ...
            case ExecutionJournalEventKind::NodeFailed:
                if (phase != NodeEventPhase::Started) {
                    result.diagnostics.error().message("execution journal validation node '" + *event.node_name + "' fails before 'node_started'").emit();
                }
                if (!event.failure_summary.has_value()) {
                    result.diagnostics.error().message("execution journal validation event '" + kind_name + "' requires failure_summary").emit();
                } else {
                    ...
                    if (event.failure_summary->kind != expected_failure_kind) {
                        result.diagnostics.error().message("execution journal validation event '" + kind_name + "' failure_summary kind does not match event kind").emit();
                    }
                }
                if (!failed_node_keys.insert(key).second) {
                    result.diagnostics.error().message("execution journal validation node '" + *event.node_name + "' duplicates failure terminal phase").emit();
                }
    ...
    for (const auto &[key, phase] : node_phases) {
        if (phase != NodeEventPhase::Completed && phase != NodeEventPhase::Failed) {
            result.diagnostics.error().message("execution journal validation node event sequence '" + key + "' does not reach terminal node phase").emit();
        }
    }

    if (terminal_workflow_kind == ExecutionJournalEventKind::WorkflowCompleted &&
        !failed_node_keys.empty()) {
        result.diagnostics.error().message("execution journal validation 'workflow_completed' must not follow failed node events").emit();
    }

    if (terminal_workflow_kind == ExecutionJournalEventKind::WorkflowFailed &&
        failed_node_keys.empty()) {
        result.diagnostics.error().message("execution journal validation 'workflow_failed' must follow at least one failed node event").emit();
    }

    if (!terminal_workflow_kind.has_value() && !failed_node_keys.empty()) {
        result.diagnostics.error().message("execution journal validation contains failed node events but does not end with 'workflow_failed'").emit();
    }
...
ExecutionJournalResult build_execution_journal(const runtime_session::RuntimeSession &session) {
    ...
    for (const auto &node_name : session.execution_order) {
        ...
        if (found == nodes_by_name.end()) {
            result.diagnostics.error().message("execution journal bootstrap execution_order node '" + node_name + "' does not exist in runtime session").emit();
            return result;
        }
        ...
        result.diagnostics.error().message("execution journal bootstrap execution_order node '" + node_name + "' is not in executable terminal status").emit();
        return result;
    }
