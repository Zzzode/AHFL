...
void validate_capability_mock_set(const dry_run::CapabilityMockSet &mock_set,
    ...
    if (mock_set.format_version != dry_run::kCapabilityMockSetFormatVersion) {
        diagnostics.error()
            .message(
                "runtime session encountered unsupported capability mock set format_version '" +
    ...
    for (const auto &mock : mock_set.mocks) {
        if (mock.capability_name.has_value() == mock.binding_key.has_value()) {
            diagnostics.error()
                .message("runtime session capability mock must specify exactly one of "
                         "'capability_name' or 'binding_key'")
                .emit();
            continue;
        }
        ...
        if (mock.result_fixture.empty()) {
            diagnostics.error()
                .message("runtime session capability mock result_fixture must not be empty")
                .emit();
        }
        ...
        if (!selector.empty() && !seen_mock_selectors.insert(selector).second) {
            diagnostics.error()
                .message("runtime session capability mock set contains duplicate mock selector '" +
                         selector + "'")
                .emit();
        }
        if (!workflow_references_mock_selector(workflow, mock)) {
            diagnostics.error()
                .message("runtime session capability mock set contains unused mock selector '" +
                         selector + "'")
                .emit();
        }
    }
...
[[nodiscard]] RuntimeSessionMockUsage make_mock_usage(const dry_run::CapabilityMock &mock) {
...
RuntimeSessionValidationResult validate_runtime_session(const RuntimeSession &session) {
    ...
    if (session.format_version != kRuntimeSessionFormatVersion) {
        result.diagnostics.error()
            .message("runtime session validation encountered unsupported format_version '" +
    ...
    if (session.source_execution_plan_format_version != handoff::kExecutionPlanFormatVersion) {
        result.diagnostics.error()
            .message("runtime session validation encountered unsupported "
    ...
    if (session.workflow_canonical_name.empty()) {
        result.diagnostics.error()
            .message("runtime session validation contains empty workflow_canonical_name")
            .emit();
    }

    if (session.session_id.empty()) {
        result.diagnostics.error()
            .message("runtime session validation contains empty session_id")
            .emit();
    }

    if (session.input_fixture.empty()) {
        result.diagnostics.error()
            .message("runtime session validation contains empty input_fixture")
            .emit();
    }
    ...
    if (!session.options.sequential_mode) {
        result.diagnostics.error()
            .message("runtime session validation currently requires sequential_mode=true")
            .emit();
    }
    ...
    for (const auto &node_name : session.execution_order) {
        if (node_name.empty()) {
            result.diagnostics.error()
                .message("runtime session validation execution_order contains empty node name")
                .emit();
            continue;
        }
        ...
        if (!execution_order_names.insert(node_name).second) {
            result.diagnostics.error()
                .message("runtime session validation execution_order contains duplicate node '" +
                         node_name + "'")
                .emit();
        }
    }
    ...
    for (const auto &node : session.nodes) {
        if (node.node_name.empty()) {
            result.diagnostics.error()
                .message("runtime session validation contains node with empty node_name")
                .emit();
            continue;
        }
        ...
        if (!nodes_by_name.emplace(node.node_name, &node).second) {
            result.diagnostics.error()
                .message("runtime session validation contains duplicate node '" + node.node_name +
                         "'")
                .emit();
            continue;
        }
        ...
        if (node.target.empty()) {
            result.diagnostics.error()
                .message("runtime session validation node '" + node.node_name +
                         "' has empty target")
                .emit();
        }
        ...
        for (const auto &binding : node.capability_bindings) {
            ...
            if (!binding_keys.insert(binding_ref).second) {
                result.diagnostics.error()
                    .message("runtime session validation node '" + node.node_name +
                             "' contains duplicate capability binding '" + binding_ref + "'")
                    .emit();
            }
        }
        ...
        for (const auto &usage : node.used_mocks) {
            if (usage.selector.empty()) {
                ...
                result.diagnostics.error()
                    .message("runtime session validation node '" + node.node_name +
                             "' contains duplicate mock selector '" + usage.selector + "'")
                    .emit();
            }
            ...
            if (usage.capability_name.has_value() == usage.binding_key.has_value()) {
                result.diagnostics.error()
                    .message("runtime session validation node '" + node.node_name +
                             "' mock usage must specify exactly one of 'capability_name' or "
                             "'binding_key'")
                    .emit();
                continue;
            }
        ...
        for (const auto &binding : node.capability_bindings) {
            ...
            if (node.status == NodeSessionStatus::Completed && !matched_mock) {
                result.diagnostics.error()
                    .message("runtime session validation node '" + node.node_name +
                             "' missing used mock for binding key '" + binding.binding_key + "'")
                    .emit();
            }
        }
    ...
    for (std::size_t index = 0; index < session.execution_order.size(); ++index) {
        ...
        if (found == nodes_by_name.end()) {
            result.diagnostics.error()
                .message("runtime session validation execution_order references unknown node '" +
                         node_name + "'")
                .emit();
            continue;
        }
        ...
        if (!node_status_is_executed(found->second->status)) {
            result.diagnostics.error()
                .message("runtime session validation execution_order node '" + node_name +
                         "' is not executed")
                .emit();
        }
    ...
    case WorkflowSessionStatus::Completed:
        ...
        if (session.execution_order.size() != session.nodes.size()) {
            result.diagnostics.error()
                .message("runtime session validation completed workflow must include every node in "
                         "execution_order")
                .emit();
        }
        if (non_completed_node_count != 0) {
            result.diagnostics.error()
                .message("runtime session validation completed workflow must not contain "
                         "non-completed nodes")
                .emit();
        }
        break;
    case WorkflowSessionStatus::Failed:
        if (!session.failure_summary.has_value()) {
            result.diagnostics.error()
                .message("runtime session validation failed workflow must carry failure summary")
                .emit();
        }
        if (failed_node_count == 0) {
            result.diagnostics.error()
                .message("runtime session validation failed workflow must contain at least one "
                         "Failed node")
                .emit();
        }
        break;
    case WorkflowSessionStatus::Partial:
        ...
        if (non_completed_node_count == 0) {
            result.diagnostics.error()
                .message("runtime session validation partial workflow must contain at least one "
                         "non-completed node")
                .emit();
        }
        break;
...
RuntimeSessionResult build_runtime_session(const handoff::ExecutionPlan &plan,
    ...
    if (!options.sequential_mode) {
        result.diagnostics.error()
            .message("runtime session bootstrap currently requires sequential_mode=true")
            .emit();
        return result;
    }
    ...
    if (workflow == nullptr) {
        result.diagnostics.error()
            .message("runtime session request workflow '" + request.workflow_canonical_name +
        ...
        for (const auto &binding : node->capability_bindings) {
            ...
            if (mock_result_fixture_requests_failure(mock->result_fixture)) {
                node_failure_summary = RuntimeFailureSummary{
                    ...
                    .node_name = node->name,
                    .message = "runtime session node '" + node->name +
                               "' failed with mock result_fixture '" + mock->result_fixture + "'",
                };
                break;
            }
        }
    ...
    if (partial_stop || terminal_failure) {
        for (const auto &node : workflow->nodes) {
            ...
            const auto node_status = terminal_failure ? NodeSessionStatus::Skipped
                                                      : (remaining_dependencies[node.name] == 0U
                                                             ? NodeSessionStatus::Ready
        ...
        result.diagnostics.error()
            .message("runtime session could not schedule all workflow nodes for '" +
