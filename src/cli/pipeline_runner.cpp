...
emit_durable_store_import_provider_conformance_report_with_diagnostics(
    ...
    const auto compatibility = build_durable_store_import_provider_compatibility_report_for_cli(
        program, metadata, mock_set, options,
...
emit_durable_store_import_provider_schema_compatibility_report_with_diagnostics(
    ...
    version_checks.push_back({
        ...
        std::string(
            ahfl::durable_store_import::kProviderProductionReadinessEvidenceFormatVersion),
        std::nullopt,
    });
    ...
    source_chain_checks.push_back({
        ...
        evidence
            ->source_durable_store_import_provider_capability_negotiation_review_format_version,
        std::string(
    ...
    source_chain_checks.push_back({
        ...
        evidence
            ->source_durable_store_import_provider_failure_taxonomy_report_format_version,
        std::string(
...
emit_durable_store_import_provider_config_bundle_validation_report_with_diagnostics(
    ...
    const auto report =
        ahfl::durable_store_import::build_provider_config_bundle_validation_report(*plan, *snapshot);
    report.diagnostics.render(std::cerr);
    if (report.has_errors() || !report.report.has_value()) {
        return 1;
    }
...
emit_durable_store_import_provider_release_evidence_archive_manifest_with_diagnostics(
    ...
    ahfl::print_durable_store_import_provider_release_evidence_archive_manifest(
        *archive.manifest, std::cout);
    return 0;
}
...
[[nodiscard]] int emit_durable_store_import_provider_approval_receipt_with_diagnostics(
    ...
    decision.approval_request_identity =
        "approval-request::" + request_result.request->session_id;
    decision.decision = ahfl::durable_store_import::ApprovalDecision::Rejected;
    decision.decision_reason =
        "default rejection: operator approval not yet provided";
    decision.rejection_details = "awaiting operator review and explicit approval";

    // 构建 approval receipt
    const auto receipt_result = ahfl::durable_store_import::build_approval_receipt(
        *request_result.request, decision);
    receipt_result.diagnostics.render(std::cerr);
    if (receipt_result.has_errors() || !receipt_result.receipt.has_value()) {
        return 1;
    }

    ahfl::print_durable_store_import_provider_approval_receipt_json(
        *receipt_result.receipt, std::cout);
    return 0;
}
...
[[nodiscard]] int emit_durable_store_import_provider_opt_in_decision_report_with_diagnostics(
    ...
    ahfl::print_durable_store_import_provider_opt_in_decision_report(
        *opt_in_result.report, std::cout);
    return 0;
}
...
emit_durable_store_import_provider_runtime_policy_report_with_diagnostics(
    ...
    const auto policy_result = ahfl::durable_store_import::build_provider_runtime_policy_report(
        *opt_in_result.report, *receipt_result.receipt, *config_report.report,
    ...
    ahfl::print_durable_store_import_provider_runtime_policy_report(
        *policy_result.report, std::cout);
    return 0;
}
...
emit_durable_store_import_provider_production_integration_dry_run_report_with_diagnostics(
    ...
    const auto dry_run_result =
        ahfl::durable_store_import::build_provider_production_integration_dry_run_report(
            *conformance.report, schema_report, *config_report.report,
            *archive_result.manifest, *receipt_result.receipt,
            *opt_in_result.report, *policy_result.report);
...
constexpr PackageCommandDispatchEntry kPackageCommandDispatch[] = {
    ...
    {CommandKind::EmitDurableStoreImportProviderApprovalReceipt,
     invoke_package_command<
         emit_durable_store_import_provider_approval_receipt_with_diagnostics>},
