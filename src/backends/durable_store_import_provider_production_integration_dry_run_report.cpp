...
void print_durable_store_import_provider_production_integration_dry_run_report(
    ...
    out << "release_evidence_archive_manifest "
    ...
    for (const auto &item : report.evidence_chain) {
        out << "evidence " << item.evidence_type << " present="
            << (item.is_present ? "true" : "false") << " valid="
            << (item.is_valid ? "true" : "false") << " fresh="
            << (item.is_fresh ? "true" : "false") << '\n';
    }
    out << "readiness_state "
        << durable_store_import::to_string_view(report.readiness_state) << '\n';
