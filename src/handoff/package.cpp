...
[[nodiscard]] std::optional<std::string> resolve_executable_name(const ExecutableIndex &index,
                                                                 ExecutableKind kind,
    ...
    if (const auto found = display.find(std::string(raw_name)); found != display.end()) {
        ...
        diagnostics.error()
            .message("package authoring field '" + std::string(field_name) + "' is ambiguous for " +
                     executable_kind_name(kind) + " '" + std::string(raw_name) + "'")
            .emit();
        return std::nullopt;
    }

    if (other_canonical.contains(std::string(raw_name)) ||
        other_display.contains(std::string(raw_name))) {
        diagnostics.error()
            .message("package authoring field '" + std::string(field_name) + "' refers to '" +
    ...
    diagnostics.error()
        .message("unknown package authoring " + executable_kind_name(kind) + " target '" +
                 std::string(raw_name) + "' in field '" + std::string(field_name) + "'")
        .emit();
    return std::nullopt;
}

[[nodiscard]] std::optional<std::string> resolve_capability_name(const CapabilityIndex &index,
                                                                 std::string_view raw_name,
                                                                 DiagnosticBag &diagnostics) {
    if (const auto found = index.canonical.find(std::string(raw_name));
        found != index.canonical.end()) {
        return *found;
    }

    if (const auto found = index.display.find(std::string(raw_name));
        found != index.display.end()) {
        if (found->second.size() == 1) {
            return found->second.front();
        }

        diagnostics.error()
            .message("package authoring capability binding is ambiguous for capability '" +
                     std::string(raw_name) + "'")
            .emit();
        return std::nullopt;
    }
    ...
    diagnostics.error()
        .message("unknown package authoring capability '" + std::string(raw_name) + "'")
        .emit();
    return std::nullopt;
}
...
PackageMetadataValidationResult validate_package_metadata(const ir::Program &program,
    ...
    for (auto &target : result.metadata.export_targets) {
        ...
        if (!seen_export_targets.insert(key).second) {
            result.diagnostics.error()
                .message("package authoring export target '" + target.canonical_name +
    ...
    for (const auto &[raw_capability_name, binding_key] : result.metadata.capability_binding_keys) {
        ...
        if (const auto found = normalized_bindings.find(*canonical_name);
            ...
            result.diagnostics.error()
                .message("package authoring capability binding for '" + *canonical_name +
                         "' is duplicated after semantic normalization")
                .emit();
            continue;
        }
    ...
    for (const auto &slot : package.capability_binding_slots) {
        for (const auto &target : slot.required_by_targets) {
            if (target.kind == ExecutableKind::Agent &&
                ...
                push_unique_binding_reference(references,
...
void validate_workflow_plan(const WorkflowPlan &workflow, DiagnosticBag &diagnostics) {
    if (workflow.workflow_canonical_name.empty()) {
        diagnostics.error()
            .message("execution plan validation contains workflow with empty canonical name")
            .emit();
    }
    ...
    for (const auto &node : workflow.nodes) {
        if (node.name.empty()) {
            diagnostics.error()
                .message("execution plan validation workflow '" + workflow_name +
    ...
    if (workflow.nodes.empty()) {
        diagnostics.error()
            .message("execution plan validation workflow '" + workflow_name + "' contains no nodes")
            .emit();
    }
    ...
    for (const auto &entry_node : workflow.entry_nodes) {
        ...
        if (found == nodes_by_name.end()) {
            diagnostics.error()
                .message("execution plan validation workflow '" + workflow_name + "' entry node '" +
    ...
    for (const auto &node : workflow.nodes) {
        if (node.target.empty()) {
            diagnostics.error()
                .message("execution plan validation workflow '" + workflow_name + "' node '" +
                         node.name + "' has empty target")
                .emit();
        }
        ...
        if (node.lifecycle.start_condition != expected_start_condition) {
            diagnostics.error()
                .message("execution plan validation workflow '" + workflow_name + "' node '" +
                         node.name + "' start condition does not match dependencies")
                .emit();
        }
        ...
        if (node.lifecycle.target_final_states.empty()) {
            diagnostics.error()
                .message("execution plan validation workflow '" + workflow_name + "' node '" +
                         node.name + "' lifecycle has no target final state")
                .emit();
        }
        for (const auto &state : node.lifecycle.target_final_states) {
            if (state.empty()) {
                diagnostics.error()
                    .message("execution plan validation workflow '" + workflow_name + "' node '" +
                             node.name + "' lifecycle contains empty target final state")
                    .emit();
            }
        }
        ...
        for (const auto &dependency : node.after) {
            if (!seen_dependencies.insert(dependency).second) {
                diagnostics.error()
                    .message("execution plan validation workflow '" + workflow_name + "' node '" +
                             node.name + "' contains duplicate after dependency '" + dependency +
                             "'")
                    .emit();
            }
            ...
            if (!node_names.contains(dependency)) {
                diagnostics.error()
                    .message("execution plan validation workflow '" + workflow_name + "' node '" +
                             node.name + "' depends on unknown node '" + dependency + "'")
                    .emit();
                continue;
            }
            ...
            if (!edge_keys.contains(edge_key(dependency, node.name))) {
                diagnostics.error()
                    .message("execution plan validation workflow '" + workflow_name + "' node '" +
                             node.name + "' after dependency '" + dependency +
        ...
        for (const auto &binding : node.capability_bindings) {
            if (binding.capability_name.empty()) {
                diagnostics.error()
                    .message("execution plan validation workflow '" + workflow_name + "' node '" +
                             node.name + "' has capability binding with empty capability name")
                    .emit();
            }
            if (binding.binding_key.empty()) {
                diagnostics.error()
                    .message("execution plan validation workflow '" + workflow_name + "' node '" +
                             node.name + "' has capability binding with empty binding key")
                    .emit();
            }
            ...
            if (!binding_refs.insert(binding_key).second) {
                diagnostics.error()
                    .message("execution plan validation workflow '" + workflow_name + "' node '" +
                             node.name + "' contains duplicate capability binding '" +
...
PackageReaderSummaryResult build_package_reader_summary(const Package &package) {
    ...
    if (result.summary.identity.has_value() &&
        ...
        result.diagnostics.error()
            .message("package reader summary encountered unsupported format_version '" +
    ...
    if (result.summary.entry_target.has_value() &&
        ...
        result.diagnostics.error()
            .message("package reader summary entry target '" +
    ...
    for (const auto &target : result.summary.export_targets) {
        if (!has_executable_target(target)) {
            result.diagnostics.error()
                .message("package reader summary export target '" + target.canonical_name +
                         "' does not exist in executable targets")
                .emit();
        }
    }
    ...
    for (const auto &slot : result.summary.capability_binding_slots) {
        if (!binding_keys.insert(slot.binding_key).second) {
            result.diagnostics.error()
                .message("package reader summary contains duplicate capability binding key '" +
                         slot.binding_key + "'")
                .emit();
        }
        ...
        for (const auto &target : slot.required_by_targets) {
            if (!has_executable_target(target)) {
                result.diagnostics.error()
                    .message("package reader summary capability binding target '" +
                             target.canonical_name + "' does not exist in executable targets")
                    .emit();
            }
        }
    }
    ...
    for (const auto &obligation : result.summary.policy_obligations) {
        if (!has_executable_target(obligation.owner_target)) {
            result.diagnostics.error()
                .message("package reader summary policy obligation owner '" +
...
build_execution_planner_bootstrap(const Package &package,
    ...
    if (workflow == nullptr) {
        if (find_agent_executable(package, workflow_canonical_name) != nullptr) {
            result.diagnostics.error()
                .message("execution planner bootstrap target '" +
                         std::string(workflow_canonical_name) + "' is not a workflow target")
                .emit();
            return result;
        }

        result.diagnostics.error()
            .message("unknown execution planner workflow target '" +
    ...
    for (const auto &node : bootstrap.nodes) {
        ...
        if (find_agent_executable(package, node.target) == nullptr) {
            result.diagnostics.error()
                .message("execution planner bootstrap node '" + node.name +
    ...
    for (const auto &entry_node : bootstrap.entry_nodes) {
        if (!node_names.contains(entry_node)) {
            result.diagnostics.error()
                .message("execution planner bootstrap entry node '" + entry_node +
                         "' does not exist in workflow '" + bootstrap.workflow_canonical_name + "'")
                .emit();
        }
    }
    ...
    for (const auto &edge : bootstrap.dependency_edges) {
        if (!node_names.contains(edge.from_node) || !node_names.contains(edge.to_node)) {
            result.diagnostics.error()
                .message("execution planner bootstrap dependency '" + edge.from_node + " -> " +
                         edge.to_node + "' refers to unknown workflow node")
                .emit();
        }
    }
        ...
        return ExecutionPlannerBootstrapResult{
            ...
            .diagnostics =
                [&]() {
                    DiagnosticBag diagnostics;
                    diagnostics.error()
                        .message("package does not define a workflow entry target for execution "
                                 "planner bootstrap")
                        .emit();
                    return diagnostics;
                }(),
...
ExecutionPlanValidationResult validate_execution_plan(const ExecutionPlan &plan) {
    ...
    if (plan.format_version != kExecutionPlanFormatVersion) {
        result.diagnostics.error()
            .message("execution plan validation encountered unsupported format_version '" +
                     plan.format_version + "'")
            .emit();
    }
    ...
    for (const auto &workflow : plan.workflows) {
        ...
        if (!workflow_names.insert(workflow.workflow_canonical_name).second) {
            result.diagnostics.error()
                .message("execution plan validation contains duplicate workflow '" +
                         workflow.workflow_canonical_name + "'")
                .emit();
        }
    }
    ...
    if (plan.workflows.empty()) {
        result.diagnostics.error()
            .message("execution plan validation contains no workflows")
            .emit();
    }
    ...
    if (plan.entry_workflow_canonical_name.has_value() &&
        ...
        result.diagnostics.error()
            .message("execution plan validation entry workflow '" +
                     *plan.entry_workflow_canonical_name + "' does not exist in workflows")
            .emit();
    }
...
ExecutionPlanResult build_execution_plan(const Package &package) {
    ...
    if (package.metadata.entry_target.has_value()) {
        ...
        if (entry_bootstrap.bootstrap.has_value()) {
            plan.entry_workflow_canonical_name = entry_bootstrap.bootstrap->workflow_canonical_name;
        }
    }
    ...
    for (const auto &target : package.executable_targets) {
        ...
        const auto bootstrap = build_execution_planner_bootstrap(package, workflow->canonical_name);
        ...
        for (const auto &node : bootstrap.bootstrap->nodes) {
            workflow_plan.nodes.push_back(WorkflowNodePlan{
                ...
                .lifecycle = node.lifecycle,
                .input_summary = node.input_summary,
                .capability_bindings = build_node_capability_binding_refs(package, node.target),
            });
        }
