...
[[nodiscard]] bool
export_artifact_entry_equals(const persistence_export::ExportArtifactEntry &lhs,
...
[[nodiscard]] bool
export_artifact_bundle_equals(const persistence_export::ExportArtifactBundle &lhs,
...
[[nodiscard]] bool
export_blocker_equals(const std::optional<persistence_export::StoreImportBlocker> &lhs,
...
[[nodiscard]] StagingArtifactKind
to_staging_artifact_kind(persistence_export::ExportArtifactKind kind) {
    switch (kind) {
...
[[nodiscard]] StagingBlockerKind
to_staging_blocker_kind(persistence_export::StoreImportBlockerKind kind) {
    switch (kind) {
...
[[nodiscard]] StagingArtifactSet
build_staging_artifact_set(const persistence_export::PersistenceExportManifest &manifest) {
...
void compare_manifest_against_expected(
    ...
    if (manifest.format_version != expected.format_version) {
        diagnostics.error()
            .message("store import descriptor bootstrap export manifest format_version does not "
                     "match derived export manifest")
            .emit();
    }
    ...
    if (manifest.source_execution_plan_format_version !=
        ...
        diagnostics.error()
            .message("store import descriptor bootstrap export manifest "
                     "source_execution_plan_format_version does not match derived export manifest")
            .emit();
    }
    ...
    if (manifest.source_runtime_session_format_version !=
        ...
        diagnostics.error()
            .message("store import descriptor bootstrap export manifest "
                     "source_runtime_session_format_version does not match derived export manifest")
            .emit();
    }
    ...
    if (manifest.source_execution_journal_format_version !=
        ...
        diagnostics.error()
            .message(
                "store import descriptor bootstrap export manifest "
                "source_execution_journal_format_version does not match derived export manifest")
            .emit();
    }

    if (manifest.source_replay_view_format_version != expected.source_replay_view_format_version) {
        diagnostics.error()
            .message("store import descriptor bootstrap export manifest "
                     "source_replay_view_format_version does not match derived export manifest")
            .emit();
    }
    ...
    if (manifest.source_scheduler_snapshot_format_version !=
        ...
        diagnostics.error()
            .message(
                "store import descriptor bootstrap export manifest "
                "source_scheduler_snapshot_format_version does not match derived export manifest")
            .emit();
    }
    ...
    if (manifest.source_checkpoint_record_format_version !=
        ...
        diagnostics.error()
            .message(
                "store import descriptor bootstrap export manifest "
                "source_checkpoint_record_format_version does not match derived export manifest")
            .emit();
    }
    ...
    if (manifest.source_persistence_descriptor_format_version !=
        ...
        diagnostics.error()
            .message("store import descriptor bootstrap export manifest "
                     "source_persistence_descriptor_format_version does not match derived export "
                     "manifest")
            .emit();
    }
    ...
    if (!package_identity_equals(manifest.source_package_identity,
        ...
        diagnostics.error()
            .message("store import descriptor bootstrap export manifest source_package_identity "
                     "does not match derived export manifest")
            .emit();
    }

    if (manifest.workflow_canonical_name != expected.workflow_canonical_name) {
        diagnostics.error()
            .message("store import descriptor bootstrap export manifest workflow_canonical_name "
                     "does not match derived export manifest")
            .emit();
    }
    ...
    if (manifest.session_id != expected.session_id) {
        diagnostics.error()
            .message("store import descriptor bootstrap export manifest session_id does not match "
                     "derived export manifest")
            .emit();
    }

    if (manifest.run_id != expected.run_id) {
        diagnostics.error()
            .message("store import descriptor bootstrap export manifest run_id does not match "
                     "derived export manifest")
            .emit();
    }
    ...
    if (manifest.input_fixture != expected.input_fixture) {
        diagnostics.error()
            .message("store import descriptor bootstrap export manifest input_fixture does not "
                     "match derived export manifest")
            .emit();
    }
    ...
    if (manifest.workflow_status != expected.workflow_status) {
        diagnostics.error()
            .message("store import descriptor bootstrap export manifest workflow_status does not "
                     "match derived export manifest")
            .emit();
    }
    ...
    if (manifest.checkpoint_status != expected.checkpoint_status) {
        diagnostics.error()
            .message("store import descriptor bootstrap export manifest checkpoint_status does not "
                     "match derived export manifest")
            .emit();
    }
    ...
    if (manifest.persistence_status != expected.persistence_status) {
        diagnostics.error()
            .message("store import descriptor bootstrap export manifest persistence_status does "
                     "not match derived export manifest")
            .emit();
    }

    if (manifest.manifest_status != expected.manifest_status) {
        diagnostics.error()
            .message("store import descriptor bootstrap export manifest manifest_status does not "
                     "match derived export manifest")
            .emit();
    }
    ...
    if (!failure_summary_equals(manifest.workflow_failure_summary,
        ...
        diagnostics.error()
            .message("store import descriptor bootstrap export manifest workflow_failure_summary "
                     "does not match derived export manifest")
            .emit();
    }
    ...
    if (manifest.export_package_identity != expected.export_package_identity) {
        diagnostics.error()
            .message("store import descriptor bootstrap export manifest export_package_identity "
                     "does not match derived export manifest")
            .emit();
    }
    ...
    if (manifest.planned_durable_identity != expected.planned_durable_identity) {
        diagnostics.error()
            .message("store import descriptor bootstrap export manifest planned_durable_identity "
                     "does not match derived export manifest")
            .emit();
    }
    ...
    if (manifest.manifest_boundary_kind != expected.manifest_boundary_kind) {
        diagnostics.error()
            .message("store import descriptor bootstrap export manifest manifest_boundary_kind "
                     "does not match derived export manifest")
            .emit();
    }
    ...
    if (!export_artifact_bundle_equals(manifest.artifact_bundle, expected.artifact_bundle)) {
        diagnostics.error()
            .message("store import descriptor bootstrap export manifest artifact_bundle does not "
                     "match derived export manifest")
            .emit();
    }
    ...
    if (manifest.manifest_ready != expected.manifest_ready) {
        diagnostics.error()
            .message("store import descriptor bootstrap export manifest manifest_ready does not "
                     "match derived export manifest")
            .emit();
    }
    ...
    if (manifest.next_required_artifact_kind != expected.next_required_artifact_kind) {
        diagnostics.error()
            .message("store import descriptor bootstrap export manifest "
                     "next_required_artifact_kind does not match derived export manifest")
            .emit();
    }

    if (!export_blocker_equals(manifest.store_import_blocker, expected.store_import_blocker)) {
        diagnostics.error()
            .message("store import descriptor bootstrap export manifest store_import_blocker does "
                     "not match derived export manifest")
            .emit();
    }
}
...
validate_store_import_descriptor(const StoreImportDescriptor &descriptor) {
    ...
    if (descriptor.format_version != kStoreImportDescriptorFormatVersion) {
        diagnostics.error()
            .message("store import descriptor format_version must be '" +
    ...
    if (descriptor.source_execution_plan_format_version != handoff::kExecutionPlanFormatVersion) {
        diagnostics.error()
            .message("store import descriptor source_execution_plan_format_version must be '" +
    ...
    if (descriptor.source_runtime_session_format_version !=
        ...
        diagnostics.error()
            .message("store import descriptor source_runtime_session_format_version must be '" +
    ...
    if (descriptor.source_execution_journal_format_version !=
        ...
        diagnostics.error()
            .message("store import descriptor source_execution_journal_format_version must be '" +
    ...
    if (descriptor.source_replay_view_format_version != replay_view::kReplayViewFormatVersion) {
        diagnostics.error()
            .message("store import descriptor source_replay_view_format_version must be '" +
    ...
    if (descriptor.source_scheduler_snapshot_format_version !=
        ...
        diagnostics.error()
            .message("store import descriptor source_scheduler_snapshot_format_version must be '" +
    ...
    if (descriptor.source_checkpoint_record_format_version !=
        ...
        diagnostics.error()
            .message("store import descriptor source_checkpoint_record_format_version must be '" +
    ...
    if (descriptor.source_export_manifest_format_version !=
        ...
        diagnostics.error()
            .message("store import descriptor source_export_manifest_format_version must be '" +
    ...
    if (descriptor.workflow_canonical_name.empty()) {
        diagnostics.error()
            .message("store import descriptor workflow_canonical_name must not be empty")
            .emit();
    }
    ...
    if (descriptor.input_fixture.empty()) {
        diagnostics.error()
            .message("store import descriptor input_fixture must not be empty")
            .emit();
    }

    if (descriptor.export_package_identity.empty()) {
        diagnostics.error()
            .message("store import descriptor export_package_identity must not be empty")
            .emit();
    }

    if (descriptor.store_import_candidate_identity.empty()) {
        diagnostics.error()
            .message("store import descriptor store_import_candidate_identity must not be empty")
            .emit();
    }

    if (descriptor.planned_durable_identity.empty()) {
        diagnostics.error()
            .message("store import descriptor planned_durable_identity must not be empty")
            .emit();
    }
    ...
    if (descriptor.staging_artifact_set.entry_count !=
        ...
        diagnostics.error()
            .message("store import descriptor staging_artifact_set entry_count must match entries "
                     "length")
            .emit();
    }
    ...
    if (descriptor.import_ready && descriptor.staging_artifact_set.entries.empty()) {
        diagnostics.error()
            .message("store import descriptor import_ready requires non-empty staging_artifact_set")
            .emit();
    }
    ...
    for (const auto &entry : descriptor.staging_artifact_set.entries) {
        if (entry.logical_artifact_name.empty()) {
            diagnostics.error()
                .message("store import descriptor staging_artifact_set entry logical_artifact_name "
                         "must not be empty")
                .emit();
        } else if (!logical_names.insert(entry.logical_artifact_name).second) {
            diagnostics.error()
                .message("store import descriptor staging_artifact_set contains duplicate "
        ...
        if (entry.source_format_version.empty()) {
            diagnostics.error()
                .message("store import descriptor staging_artifact_set entry source_format_version "
                         "must not be empty")
                .emit();
        }
    }
    ...
    if (descriptor.import_ready && descriptor.next_required_staging_artifact_kind.has_value()) {
        diagnostics.error()
            .message("store import descriptor cannot contain next_required_staging_artifact_kind "
                     "when import_ready is true")
            .emit();
    }
    ...
    if (descriptor.staging_blocker.has_value()) {
        if (descriptor.staging_blocker->message.empty()) {
            diagnostics.error()
                .message("store import descriptor staging_blocker message must not be empty")
                .emit();
        }
        ...
        if (descriptor.staging_blocker->logical_artifact_name.has_value() &&
            ...
            diagnostics.error()
                .message("store import descriptor staging_blocker logical_artifact_name must not "
                         "be empty")
                .emit();
        }
    }
    ...
    if (descriptor.descriptor_status == StoreImportDescriptorStatus::ReadyToImport &&
        ...
        diagnostics.error()
            .message("store import descriptor ReadyToImport status requires import_ready")
            .emit();
    }
    ...
    if (descriptor.descriptor_status == StoreImportDescriptorStatus::ReadyToImport &&
        ...
        diagnostics.error()
            .message("store import descriptor ReadyToImport status requires ReadyToImport "
                     "manifest_status")
            .emit();
    }
    ...
    if (descriptor.descriptor_status == StoreImportDescriptorStatus::ReadyToImport &&
        descriptor.staging_blocker.has_value()) {
        diagnostics.error()
            .message("store import descriptor ReadyToImport status cannot have staging_blocker")
            .emit();
    }
    ...
    if (descriptor.descriptor_status == StoreImportDescriptorStatus::Blocked &&
        descriptor.import_ready) {
        diagnostics.error()
            .message("store import descriptor Blocked status cannot be import_ready")
            .emit();
    }
    ...
    if (descriptor.descriptor_status == StoreImportDescriptorStatus::Blocked &&
        ...
        diagnostics.error()
            .message("store import descriptor Blocked status requires Blocked manifest_status")
            .emit();
    }
    ...
    if (descriptor.descriptor_status == StoreImportDescriptorStatus::TerminalCompleted &&
        !descriptor.import_ready) {
        diagnostics.error()
            .message("store import descriptor TerminalCompleted status requires import_ready")
            .emit();
    }
    ...
    if (descriptor.descriptor_status == StoreImportDescriptorStatus::TerminalCompleted &&
        ...
        diagnostics.error()
            .message("store import descriptor TerminalCompleted status requires TerminalCompleted "
                     "manifest_status")
            .emit();
    }
    ...
    if (descriptor.descriptor_status == StoreImportDescriptorStatus::TerminalCompleted &&
        descriptor.staging_blocker.has_value()) {
        diagnostics.error()
            .message("store import descriptor TerminalCompleted status cannot have staging_blocker")
            .emit();
    }
    ...
    if (descriptor.descriptor_status == StoreImportDescriptorStatus::TerminalCompleted &&
        ...
        diagnostics.error()
            .message("store import descriptor TerminalCompleted status cannot have "
                     "next_required_staging_artifact_kind")
            .emit();
    }
    ...
    if ((descriptor.descriptor_status == StoreImportDescriptorStatus::TerminalFailed ||
        ...
        !descriptor.staging_blocker.has_value()) {
        diagnostics.error()
            .message("store import descriptor terminal blocked status requires staging_blocker")
            .emit();
    }
    ...
    if ((descriptor.descriptor_status == StoreImportDescriptorStatus::TerminalFailed ||
        ...
        descriptor.import_ready) {
        diagnostics.error()
            .message("store import descriptor terminal blocked status cannot be import_ready")
            .emit();
    }
    ...
    if ((descriptor.descriptor_status == StoreImportDescriptorStatus::TerminalFailed ||
        ...
        descriptor.next_required_staging_artifact_kind.has_value()) {
        diagnostics.error()
            .message("store import descriptor terminal blocked status cannot have "
                     "next_required_staging_artifact_kind")
            .emit();
    }
    ...
    if (descriptor.descriptor_status == StoreImportDescriptorStatus::TerminalFailed &&
        ...
        diagnostics.error()
            .message("store import descriptor TerminalFailed status requires TerminalFailed "
                     "manifest_status")
            .emit();
    }
    ...
    if (descriptor.descriptor_status == StoreImportDescriptorStatus::TerminalPartial &&
        ...
        diagnostics.error()
            .message("store import descriptor TerminalPartial status requires TerminalPartial "
                     "manifest_status")
            .emit();
    }
    ...
    if (descriptor.descriptor_boundary_kind == StoreImportBoundaryKind::AdapterConsumable &&
        ...
        descriptor.descriptor_status != StoreImportDescriptorStatus::TerminalCompleted) {
        diagnostics.error()
            .message("store import descriptor adapter-consumable boundary requires ready or "
                     "completed descriptor_status")
            .emit();
    }
    ...
    if (descriptor.workflow_status == runtime_session::WorkflowSessionStatus::Completed &&
        descriptor.descriptor_status != StoreImportDescriptorStatus::TerminalCompleted) {
        diagnostics.error()
            .message("store import descriptor completed workflow_status requires TerminalCompleted "
                     "descriptor_status")
            .emit();
    }
    ...
    if (descriptor.workflow_status == runtime_session::WorkflowSessionStatus::Failed &&
        ...
        diagnostics.error()
            .message("store import descriptor failed workflow_status requires TerminalFailed "
                     "descriptor_status")
            .emit();
    }
    ...
    if (descriptor.checkpoint_status ==
        ...
        descriptor.descriptor_status != StoreImportDescriptorStatus::TerminalCompleted) {
        diagnostics.error()
            .message("store import descriptor TerminalCompleted checkpoint_status requires "
    ...
    if (descriptor.checkpoint_status == checkpoint_record::CheckpointRecordStatus::TerminalFailed &&
        descriptor.descriptor_status != StoreImportDescriptorStatus::TerminalFailed) {
        diagnostics.error()
            .message("store import descriptor TerminalFailed checkpoint_status requires "
    ...
    if (descriptor.checkpoint_status ==
        ...
        descriptor.descriptor_status != StoreImportDescriptorStatus::TerminalPartial) {
        diagnostics.error()
            .message("store import descriptor TerminalPartial checkpoint_status requires "
    ...
    if (descriptor.persistence_status ==
        ...
        descriptor.descriptor_status != StoreImportDescriptorStatus::TerminalCompleted) {
        diagnostics.error()
            .message("store import descriptor TerminalCompleted persistence_status requires "
                     "TerminalCompleted descriptor_status")
            .emit();
    }
    ...
    if (descriptor.persistence_status ==
        ...
        descriptor.descriptor_status != StoreImportDescriptorStatus::TerminalFailed) {
        diagnostics.error()
            .message("store import descriptor TerminalFailed persistence_status requires "
                     "TerminalFailed descriptor_status")
            .emit();
    }
    ...
    if (descriptor.persistence_status ==
        ...
        descriptor.descriptor_status != StoreImportDescriptorStatus::TerminalPartial) {
        diagnostics.error()
            .message("store import descriptor TerminalPartial persistence_status requires "
                     "TerminalPartial descriptor_status")
            .emit();
    }
    ...
    if (descriptor.workflow_status == runtime_session::WorkflowSessionStatus::Partial &&
        ...
        descriptor.descriptor_status != StoreImportDescriptorStatus::TerminalPartial) {
        diagnostics.error()
            .message("store import descriptor partial workflow_status must map to ReadyToImport, "
                     "Blocked, or TerminalPartial descriptor_status")
            .emit();
    }
...
StoreImportDescriptorResult build_store_import_descriptor(
