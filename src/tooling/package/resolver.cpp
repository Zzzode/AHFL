...
void Resolver::add_dependency(Dependency dep) {
...
SemVer Resolver::parse_version(const std::string &version_str) {
    ...
    if (std::getline(stream, segment, '.')) {
        auto [ptr, ec] =
            std::from_chars(segment.data(), segment.data() + segment.size(), result.major);
        (void)ptr;
        (void)ec;
    }
    if (std::getline(stream, segment, '.')) {
        auto [ptr, ec] =
            std::from_chars(segment.data(), segment.data() + segment.size(), result.minor);
        (void)ptr;
        (void)ec;
    }
    if (std::getline(stream, segment, '.')) {
        auto [ptr, ec] =
            std::from_chars(segment.data(), segment.data() + segment.size(), result.patch);
        (void)ptr;
        (void)ec;
    }
        ...
        for (std::size_t j = i + 1; j < result.resolved.size(); ++j) {
            if (result.resolved[i].name == result.resolved[j].name) {
                if (compare(result.resolved[i].version, result.resolved[j].version) != 0) {
                    ...
                    result.error_message = "Version conflict for '" + result.resolved[i].name +
                                           "': " + format_version(result.resolved[i].version) +
                                           " vs " + format_version(result.resolved[j].version);
                    return result;
                }
            }
        }
