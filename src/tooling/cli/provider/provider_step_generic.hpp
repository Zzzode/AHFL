...
#define AHFL_DEFINE_PROVIDER_STEP_1(fn_name, return_type, dep_kind, domain_fn, result_field)       \
    [[nodiscard]] std::optional<ahfl::durable_store_import::return_type>                           \
    fn_name(const ahfl::ir::Program & /*program*/,                                                 \
            const ahfl::handoff::PackageMetadata & /*metadata*/,                                   \
            const ahfl::dry_run::CapabilityMockSet & /*mock_set*/,                                 \
            const CommandLineOptions & /*options*/,                                                 \
            std::string_view /*command_name*/,                                                      \
            ProviderPipelineCache &cache) {                                                         \
        const auto *dep = cache.get_##dep_kind();                                                  \
...
#define AHFL_DEFINE_PROVIDER_STEP_2(fn_name, return_type, dep1_kind, dep2_kind, domain_fn, result_field) \
    [[nodiscard]] std::optional<ahfl::durable_store_import::return_type>                                 \
    fn_name(const ahfl::ir::Program & /*program*/,                                                       \
            const ahfl::handoff::PackageMetadata & /*metadata*/,                                         \
            const ahfl::dry_run::CapabilityMockSet & /*mock_set*/,                                       \
            ...
            std::string_view /*command_name*/,                                                            \
            ProviderPipelineCache &cache) {                                                               \
        const auto *dep1 = cache.get_##dep1_kind();                                                      \
        if (dep1 == nullptr) { return std::nullopt; }                                                    \
        const auto *dep2 = cache.get_##dep2_kind();                                                      \
        if (dep2 == nullptr) { return std::nullopt; }                                                    \
