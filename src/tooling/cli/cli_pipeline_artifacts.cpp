...
const ahfl::execution_journal::ExecutionJournal *CliPipelineArtifacts::execution_journal() {
    return execution_journal_.get(
        [&]() -> std::optional<ahfl::execution_journal::ExecutionJournal> {
    ...
    return scheduler_snapshot_.get(
        [&]() -> std::optional<ahfl::scheduler_snapshot::SchedulerSnapshot> {
            ...
            if (plan == nullptr || session == nullptr || journal == nullptr || replay == nullptr)
                return std::nullopt;
            return build_scheduler_snapshot_for_cli(
                *plan, *session, *journal, *replay, inputs_.err);
        });
}
    ...
    return checkpoint_record_.get(
        [&]() -> std::optional<ahfl::checkpoint_record::CheckpointRecord> {
            ...
            if (plan == nullptr || session == nullptr || journal == nullptr || replay == nullptr ||
                snapshot == nullptr)
                return std::nullopt;
            return build_checkpoint_record_for_cli(
                *plan, *session, *journal, *replay, *snapshot, inputs_.err);
        });
}
    ...
    return persistence_descriptor_.get(
        [&]() -> std::optional<ahfl::persistence_descriptor::CheckpointPersistenceDescriptor> {
            ...
            if (plan == nullptr || session == nullptr || journal == nullptr || replay == nullptr ||
                ...
                return std::nullopt;
            return build_persistence_descriptor_for_cli(
                *plan, *session, *journal, *replay, *snapshot, *record, inputs_.err);
        });
}
    ...
    return export_manifest_.get(
        [&]() -> std::optional<ahfl::persistence_export::PersistenceExportManifest> {
            ...
            if (plan == nullptr || session == nullptr || journal == nullptr || replay == nullptr ||
                ...
                return std::nullopt;
            return build_export_manifest_for_cli(
                *plan, *session, *journal, *replay, *snapshot, *record, *descriptor, inputs_.err);
        });
}
    ...
    return store_import_descriptor_.get(
        [&]() -> std::optional<ahfl::store_import::StoreImportDescriptor> {
            ...
            if (plan == nullptr || session == nullptr || journal == nullptr || replay == nullptr ||
                ...
                manifest == nullptr)
                return std::nullopt;
            return build_store_import_descriptor_for_cli(*plan,
                                                         *session,
                                                         *journal,
                                                         *replay,
                                                         *snapshot,
