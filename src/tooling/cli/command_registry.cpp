...
constexpr CommandSpec kCommandSpecs[] = {
    ...
    routed_command(
        CommandKind::PackageArchive, "package-archive", ActionGroup::Package, "archive", 103, 100),
    routed_command(
    ...
    emit_command(CommandKind::EmitPackageReview,
                 ...
                 102,
                 11),
    emit_command(CommandKind::EmitPublicApi,
                 "emit-public-api",
                 "public-api",
                 "public-api",
                 100,
                 ...
                 103,
                 12),
    emit_command(CommandKind::EmitPublicApiDocs,
                 "emit-public-api-docs",
                 "public-api-docs",
                 "public-api-docs",
                 101,
                 98,
                 104,
                 13),
    emit_command(CommandKind::EmitPublicApiDiff,
                 "emit-public-api-diff",
                 "public-api-diff",
                 "public-api-diff",
                 102,
                 99,
    ...
    emit_command(CommandKind::EmitAssuranceJson,
                 ...
                 114,
                 118),
    emit_command(CommandKind::EmitK8sCrd, "emit-k8s-crd", "k8s-crd", "k8s-crd", 119, 115, 119),
...
SelectedAction selected_action_from_options(const CommandLineOptions &options,
    ...
    (void)options;
    return SelectedAction{.command = effective_command};
}
...
bool selected_action_supports_package(const SelectedAction &action) {
    return action.command.has_value() && is_package_supported_command(*action.command);
}

bool selected_action_supports_capability_inputs(const SelectedAction &action) {
    return action.command.has_value() && supports_capability_inputs(action.command);
}

bool selected_action_requires_package(const SelectedAction &action) {
    return action.command.has_value() && is_command_requiring_package(*action.command);
}
