...
inline constexpr ErrorCode<DiagnosticCategory::Validation> kValidationDiagnosticCode{"DSI_RECEIPT"};
...
void validate_receipt_blocker(const Receipt &receipt, DiagnosticBag &diagnostics) {
    ...
    if (blocker.required_capability.has_value() &&
        ...
        emit_validation_error(diagnostics,
                              "durable store import decision receipt receipt_blocker "
...
build_receipt_identity(const std::optional<handoff::PackageIdentity> &source_package_identity,
    ...
    return "durable-store-import-receipt::" + identity_anchor + "::" + std::string(session_id) +
           "::" + receipt_outcome_slug(outcome);
}
...
ReceiptValidationResult validate_receipt(const Receipt &receipt) {
    ...
    if (receipt.source_durable_store_import_decision_format_version != kDecisionFormatVersion) {
        emit_validation_error(diagnostics,
                              "durable store import decision receipt "
    ...
    if (receipt.source_persistence_descriptor_format_version !=
        ...
        emit_validation_error(
            diagnostics,
            "durable store import decision receipt source_persistence_descriptor_format_version "
            "must be '" +
    ...
    if (receipt.workflow_canonical_name.empty()) {
        emit_validation_error(
            diagnostics,
            "durable store import decision receipt workflow_canonical_name must not "
            "be empty");
    }
    ...
    if (receipt.input_fixture.empty()) {
        emit_validation_error(
            diagnostics, "durable store import decision receipt input_fixture must not be empty");
    }

    if (receipt.export_package_identity.empty()) {
        emit_validation_error(
            diagnostics,
            "durable store import decision receipt export_package_identity must not "
            "be empty");
    }
    ...
    if (receipt.planned_durable_identity.empty()) {
        emit_validation_error(diagnostics,
                              "durable store import decision receipt planned_durable_identity must "
                              "not be empty");
    }
    ...
    if (receipt.accepted_for_receipt_archive && receipt.receipt_blocker.has_value()) {
        emit_validation_error(
            diagnostics,
            "durable store import decision receipt cannot contain receipt_blocker "
    ...
    if (receipt.accepted_for_receipt_archive &&
        ...
        emit_validation_error(diagnostics,
                              "durable store import decision receipt cannot contain "
    ...
    if (!receipt.accepted_for_receipt_archive && !receipt.receipt_blocker.has_value()) {
        emit_validation_error(diagnostics,
                              "durable store import decision receipt must contain receipt_blocker "
    ...
    if (receipt.accepted_for_receipt_archive &&
        ...
        emit_validation_error(diagnostics,
                              "durable store import decision receipt accepted_for_receipt_archive "
    ...
    if (receipt.receipt_boundary_kind == ReceiptBoundaryKind::AdapterReceiptConsumable &&
        ...
        emit_validation_error(diagnostics,
                              "durable store import decision receipt adapter-receipt-consumable "
    ...
    case ReceiptStatus::ReadyForArchive:
        if (receipt.receipt_outcome != ReceiptOutcome::ArchiveAcceptedDecision) {
            emit_validation_error(diagnostics,
                                  "durable store import decision receipt ReadyForArchive status "
    ...
    case ReceiptStatus::Blocked:
        if (receipt.receipt_outcome != ReceiptOutcome::BlockBlockedDecision) {
            emit_validation_error(diagnostics,
                                  "durable store import decision receipt Blocked status requires "
                                  "BlockBlockedDecision outcome");
        }
        if (receipt.accepted_for_receipt_archive) {
            emit_validation_error(diagnostics,
                                  "durable store import decision receipt Blocked status cannot be "
    ...
    case ReceiptStatus::Deferred:
        if (receipt.receipt_outcome != ReceiptOutcome::DeferPartialDecision) {
            emit_validation_error(diagnostics,
                                  "durable store import decision receipt Deferred status requires "
                                  "DeferPartialDecision outcome");
        }
        if (receipt.accepted_for_receipt_archive) {
            emit_validation_error(diagnostics,
                                  "durable store import decision receipt Deferred status cannot be "
                                  "accepted_for_receipt_archive");
        }
    ...
    case ReceiptStatus::Rejected:
        if (receipt.receipt_outcome != ReceiptOutcome::RejectFailedDecision) {
            emit_validation_error(diagnostics,
                                  "durable store import decision receipt Rejected status requires "
                                  "RejectFailedDecision outcome");
        }
        if (receipt.accepted_for_receipt_archive) {
            emit_validation_error(diagnostics,
                                  "durable store import decision receipt Rejected status cannot be "
                                  "accepted_for_receipt_archive");
        }
    ...
    if (receipt.workflow_status == runtime_session::WorkflowSessionStatus::Completed &&
        ...
        emit_validation_error(diagnostics,
                              "durable store import decision receipt completed workflow_status "
    ...
    if (receipt.workflow_status == runtime_session::WorkflowSessionStatus::Failed &&
        ...
        emit_validation_error(
            diagnostics,
            "durable store import decision receipt failed workflow_status requires "
    ...
    if (receipt.workflow_status == runtime_session::WorkflowSessionStatus::Partial &&
        ...
        emit_validation_error(
            diagnostics,
            "durable store import decision receipt partial workflow_status must map "
...
ReceiptResult build_receipt(const Decision &decision) {
    ...
    Receipt receipt{
        ...
        .source_checkpoint_record_format_version = decision.source_checkpoint_record_format_version,
        .source_persistence_descriptor_format_version =
            decision.source_persistence_descriptor_format_version,
        .source_export_manifest_format_version = decision.source_export_manifest_format_version,
    ...
    receipt.durable_store_import_receipt_identity =
        build_receipt_identity(receipt.source_package_identity,
