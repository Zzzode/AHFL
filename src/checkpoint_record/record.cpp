...
CheckpointRecordValidationResult validate_checkpoint_record(const CheckpointRecord &record) {
    CheckpointRecordValidationResult result;
    auto &diagnostics = result.diagnostics;

    if (record.format_version != kCheckpointRecordFormatVersion) {
        diagnostics.error()
            .message("checkpoint record format_version must be '" +
    ...
    if (record.source_execution_plan_format_version != handoff::kExecutionPlanFormatVersion) {
        diagnostics.error()
            .message("checkpoint record source_execution_plan_format_version must be '" +
    ...
    if (record.source_runtime_session_format_version !=
        ...
        diagnostics.error()
            .message("checkpoint record source_runtime_session_format_version must be '" +
    ...
    if (record.source_execution_journal_format_version !=
        ...
        diagnostics.error()
            .message("checkpoint record source_execution_journal_format_version must be '" +
    ...
    if (record.source_replay_view_format_version != replay_view::kReplayViewFormatVersion) {
        diagnostics.error()
            .message("checkpoint record source_replay_view_format_version must be '" +
    ...
    if (record.source_scheduler_snapshot_format_version !=
        ...
        diagnostics.error()
            .message("checkpoint record source_scheduler_snapshot_format_version must be '" +
    ...
    if (record.workflow_canonical_name.empty()) {
        diagnostics.error()
            .message("checkpoint record workflow_canonical_name must not be empty")
            .emit();
    }
    ...
    for (const auto &node_name : record.execution_order) {
        if (node_name.empty()) {
            diagnostics.error()
                .message("checkpoint record execution_order contains empty node name")
                .emit();
            continue;
        }
        ...
        if (!execution_nodes.insert(node_name).second) {
            diagnostics.error()
                .message("checkpoint record execution_order contains duplicate node '" + node_name +
                         "'")
                .emit();
        }
    }
    ...
    if (record.cursor.persistable_prefix_size != record.cursor.persistable_prefix.size()) {
        diagnostics.error()
            .message("checkpoint record cursor persistable_prefix_size must match "
    ...
    for (std::size_t index = 0; index < record.cursor.persistable_prefix.size(); ++index) {
        if (record.cursor.persistable_prefix[index] != record.execution_order[index]) {
            diagnostics.error()
                .message("checkpoint record cursor persistable_prefix must be a prefix of "
                         "execution_order")
                .emit();
            break;
        }
    ...
    if (record.cursor.resume_candidate_node_name.has_value()) {
        if (record.cursor.resume_candidate_node_name->empty()) {
            diagnostics.error()
                .message("checkpoint record cursor resume_candidate_node_name must not be empty")
                .emit();
        } else if (!execution_nodes.contains(*record.cursor.resume_candidate_node_name)) {
            diagnostics.error()
                .message("checkpoint record cursor resume_candidate_node_name '" +
    ...
    if (record.resume_blocker.has_value()) {
        if (record.resume_blocker->message.empty()) {
            diagnostics.error()
                .message("checkpoint record resume_blocker message must not be empty")
                .emit();
        }
        ...
        if (record.resume_blocker->node_name.has_value() &&
            ...
            diagnostics.error()
                .message("checkpoint record resume_blocker node_name must not be empty")
                .emit();
        }
    }
    ...
    if (!record.checkpoint_friendly_source &&
        ...
        diagnostics.error()
            .message("checkpoint record durable-adjacent basis requires checkpoint_friendly_source")
            .emit();
    }
    ...
    if (record.checkpoint_status == CheckpointRecordStatus::ReadyToPersist &&
        ...
        diagnostics.error()
            .message("checkpoint record ReadyToPersist status requires cursor resume_ready")
            .emit();
    }
    ...
    if (record.checkpoint_status == CheckpointRecordStatus::ReadyToPersist &&
        ...
        diagnostics.error()
            .message("checkpoint record ReadyToPersist status requires resume_candidate_node_name")
            .emit();
    }
    ...
    if (record.checkpoint_status == CheckpointRecordStatus::TerminalCompleted &&
        ...
        diagnostics.error()
            .message("checkpoint record TerminalCompleted status requires full persistable_prefix")
            .emit();
    }
    ...
    if (record.checkpoint_status == CheckpointRecordStatus::TerminalCompleted &&
        record.resume_blocker.has_value()) {
        diagnostics.error()
            .message("checkpoint record TerminalCompleted status cannot have resume_blocker")
            .emit();
    }
    ...
    if ((record.checkpoint_status == CheckpointRecordStatus::TerminalFailed ||
        ...
        !record.resume_blocker.has_value()) {
        diagnostics.error()
            .message("checkpoint record terminal blocked status requires resume_blocker")
            .emit();
    }
    ...
    if (record.checkpoint_status == CheckpointRecordStatus::TerminalFailed &&
        ...
        diagnostics.error()
            .message("checkpoint record TerminalFailed status requires workflow_failure_summary")
            .emit();
    }
    ...
    if (record.checkpoint_status == CheckpointRecordStatus::TerminalFailed &&
        record.cursor.resume_ready) {
        diagnostics.error()
            .message("checkpoint record TerminalFailed status cannot be resume_ready")
            .emit();
    }
    ...
    if (record.checkpoint_status == CheckpointRecordStatus::TerminalPartial &&
        record.cursor.resume_ready) {
        diagnostics.error()
            .message("checkpoint record TerminalPartial status cannot be resume_ready")
            .emit();
    }
    ...
    if (record.workflow_status == runtime_session::WorkflowSessionStatus::Completed &&
        record.checkpoint_status != CheckpointRecordStatus::TerminalCompleted) {
        diagnostics.error()
            .message("checkpoint record completed workflow_status requires TerminalCompleted "
                     "checkpoint_status")
            .emit();
    }
    ...
    if (record.workflow_status == runtime_session::WorkflowSessionStatus::Failed &&
        ...
        diagnostics.error()
            .message("checkpoint record failed workflow_status requires TerminalFailed "
                     "checkpoint_status")
            .emit();
    }
    ...
    if (record.snapshot_status == scheduler_snapshot::SchedulerSnapshotStatus::TerminalCompleted &&
        record.checkpoint_status != CheckpointRecordStatus::TerminalCompleted) {
        diagnostics.error()
            .message("checkpoint record terminal-completed snapshot_status requires "
                     "TerminalCompleted checkpoint_status")
            .emit();
    }
    ...
    if (record.snapshot_status == scheduler_snapshot::SchedulerSnapshotStatus::TerminalFailed &&
        record.checkpoint_status != CheckpointRecordStatus::TerminalFailed) {
        diagnostics.error()
            .message("checkpoint record terminal-failed snapshot_status requires TerminalFailed "
                     "checkpoint_status")
            .emit();
    }
    ...
    if (record.snapshot_status == scheduler_snapshot::SchedulerSnapshotStatus::TerminalPartial &&
        record.checkpoint_status != CheckpointRecordStatus::TerminalPartial) {
        diagnostics.error()
            .message("checkpoint record terminal-partial snapshot_status requires TerminalPartial "
                     "checkpoint_status")
            .emit();
    }
...
build_checkpoint_record(const handoff::ExecutionPlan &plan,
    ...
    if (session.source_execution_plan_format_version != plan.format_version) {
        result.diagnostics.error()
            .message("checkpoint record bootstrap runtime session "
    ...
    if (journal.source_execution_plan_format_version != plan.format_version) {
        result.diagnostics.error()
            .message("checkpoint record bootstrap execution journal "
                     "source_execution_plan_format_version does not match execution plan")
            .emit();
    }
    ...
    if (replay.source_execution_plan_format_version != plan.format_version) {
        result.diagnostics.error()
            .message("checkpoint record bootstrap replay view source_execution_plan_format_version "
    ...
    if (replay.source_runtime_session_format_version != session.format_version) {
        result.diagnostics.error()
            .message("checkpoint record bootstrap replay view "
                     "source_runtime_session_format_version does not match runtime session")
            .emit();
    }
    ...
    if (snapshot.source_execution_plan_format_version != plan.format_version) {
        result.diagnostics.error()
            .message("checkpoint record bootstrap scheduler snapshot "
                     "source_execution_plan_format_version does not match execution plan")
            .emit();
    }
    ...
    if (!package_identity_equals(plan.source_package_identity, session.source_package_identity)) {
        result.diagnostics.error()
            .message("checkpoint record bootstrap runtime session source_package_identity does not "
    ...
    if (!package_identity_equals(plan.source_package_identity, journal.source_package_identity)) {
        result.diagnostics.error()
            .message("checkpoint record bootstrap execution journal source_package_identity does "
    ...
    if (!package_identity_equals(plan.source_package_identity, replay.source_package_identity)) {
        result.diagnostics.error()
            .message("checkpoint record bootstrap replay view source_package_identity does not "
                     "match execution plan")
            .emit();
    }
    ...
    if (!package_identity_equals(plan.source_package_identity, snapshot.source_package_identity)) {
        result.diagnostics.error()
            .message("checkpoint record bootstrap scheduler snapshot source_package_identity does "
                     "not match execution plan")
            .emit();
    }
    ...
    if (session.workflow_canonical_name != journal.workflow_canonical_name) {
        result.diagnostics.error()
            .message("checkpoint record bootstrap execution journal workflow_canonical_name does "
    ...
    if (session.workflow_canonical_name != replay.workflow_canonical_name) {
        result.diagnostics.error()
            .message("checkpoint record bootstrap replay view workflow_canonical_name does not "
    ...
    if (session.workflow_canonical_name != snapshot.workflow_canonical_name) {
        result.diagnostics.error()
            .message("checkpoint record bootstrap scheduler snapshot workflow_canonical_name does "
                     "not match runtime session")
            .emit();
    }
    ...
    if (session.session_id != journal.session_id) {
        result.diagnostics.error()
            .message("checkpoint record bootstrap execution journal session_id does not match "
                     "runtime session")
            .emit();
    }
    ...
    if (session.session_id != snapshot.session_id) {
        result.diagnostics.error()
            .message("checkpoint record bootstrap scheduler snapshot session_id does not match "
                     "runtime session")
            .emit();
    }
    ...
    if (session.run_id != journal.run_id) {
        result.diagnostics.error()
            .message("checkpoint record bootstrap execution journal run_id does not match runtime "
                     "session")
            .emit();
    }
    ...
    if (session.run_id != snapshot.run_id) {
        result.diagnostics.error()
            .message("checkpoint record bootstrap scheduler snapshot run_id does not match runtime "
                     "session")
            .emit();
    }
    ...
    if (session.input_fixture != replay.input_fixture) {
        result.diagnostics.error()
            .message("checkpoint record bootstrap replay view input_fixture does not match runtime "
                     "session")
            .emit();
    }
    ...
    if (session.input_fixture != snapshot.input_fixture) {
        result.diagnostics.error()
            .message("checkpoint record bootstrap scheduler snapshot input_fixture does not match "
                     "runtime session")
            .emit();
    }
    ...
    if (session.workflow_status != replay.workflow_status) {
        result.diagnostics.error()
            .message("checkpoint record bootstrap replay view workflow_status does not match "
                     "runtime session")
            .emit();
    }
    ...
    if (session.workflow_status != snapshot.workflow_status) {
        result.diagnostics.error()
            .message("checkpoint record bootstrap scheduler snapshot workflow_status does not "
                     "match runtime session")
            .emit();
    }
    ...
    if (!failure_summary_equals(session.failure_summary, replay.workflow_failure_summary)) {
        result.diagnostics.error()
            .message("checkpoint record bootstrap replay view workflow_failure_summary does not "
                     "match runtime session")
            .emit();
    }
    ...
    if (!failure_summary_equals(session.failure_summary, snapshot.workflow_failure_summary)) {
        result.diagnostics.error()
            .message("checkpoint record bootstrap scheduler snapshot workflow_failure_summary does "
                     "not match runtime session")
            .emit();
    }
    ...
    if (!replay.consistency.plan_matches_session || !replay.consistency.session_matches_journal ||
        ...
        result.diagnostics.error()
            .message("checkpoint record bootstrap replay view consistency must hold before "
                     "building checkpoint record")
            .emit();
    }
    ...
    if (workflow == nullptr) {
        result.diagnostics.error()
            .message("checkpoint record bootstrap workflow '" + session.workflow_canonical_name +
    ...
    if (!vector_equals(snapshot.execution_order, plan_execution_order)) {
        result.diagnostics.error()
            .message("checkpoint record bootstrap scheduler snapshot execution_order does not "
    ...
    if (!vector_equals(replay.execution_order, session.execution_order)) {
        result.diagnostics.error()
            .message("checkpoint record bootstrap replay view execution_order does not match "
                     "runtime session")
            .emit();
    }
    ...
    if (!is_prefix(replay.execution_order, snapshot.execution_order)) {
        result.diagnostics.error()
            .message("checkpoint record bootstrap replay view execution_order must be a prefix of "
    ...
    if (!is_prefix(snapshot.cursor.completed_prefix, replay.execution_order)) {
        result.diagnostics.error()
            .message("checkpoint record bootstrap scheduler snapshot completed_prefix must be a "
                     "prefix of replay view execution_order")
            .emit();
    }
