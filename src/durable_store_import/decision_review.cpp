...
void validate_decision_blocker(const DecisionBlocker &blocker, DiagnosticBag &diagnostics) {
    if (blocker.message.empty()) {
        emit_validation_error(diagnostics,
                              "durable store import decision review summary decision_blocker "
...
[[nodiscard]] DecisionReviewNextActionKind next_action_for_decision(const Decision &decision) {
...
[[nodiscard]] std::string adapter_contract_summary_for_decision(const Decision &decision) {
    switch (decision.decision_boundary_kind) {
    case DecisionBoundaryKind::LocalContractOnly:
        return "local durable adapter contract reasoning only; real receipt and executor ABI not "
               "yet promised";
    case DecisionBoundaryKind::AdapterDecisionConsumable:
        return "adapter-decision-consumable contract shape available; real receipt and executor "
               "ABI still not promised";
    }

    return "invalid durable adapter decision boundary";
}

[[nodiscard]] std::string decision_preview_for_decision(const Decision &decision) {
    switch (decision.decision_status) {
    case DecisionStatus::Accepted:
        if (decision.request_status == RequestStatus::TerminalCompleted) {
            return "workflow already completed; durable adapter decision is retained for archival "
                   "review";
        }
    ...
    case DecisionStatus::Blocked:
        return "durable adapter decision remains blocked until required capability '" +
               ...
               "' is available";
    case DecisionStatus::Deferred:
        return "partial workflow durable adapter decision is deferred until partial state "
    ...
    case DecisionStatus::Rejected:
        return decision.workflow_failure_summary.has_value() &&
                   ...
                   ? "workflow failed at node '" + *decision.workflow_failure_summary->node_name +
...
[[nodiscard]] std::string next_step_recommendation_for_decision(const Decision &decision) {
    switch (decision.decision_status) {
    case DecisionStatus::Accepted:
        if (decision.request_status == RequestStatus::TerminalCompleted) {
    ...
    case DecisionStatus::Blocked:
        return "add required adapter capability before claiming durable adapter contract "
               "acceptance";
    case DecisionStatus::Deferred:
...
validate_decision_review_summary(const DecisionReviewSummary &summary) {
    ...
    if (summary.format_version != kDecisionReviewFormatVersion) {
        emit_validation_error(
            diagnostics,
            "durable store import decision review summary format_version must be '" +
    ...
    if (summary.source_durable_store_import_decision_format_version != kDecisionFormatVersion) {
        emit_validation_error(diagnostics,
                              "durable store import decision review summary "
    ...
    if (summary.workflow_canonical_name.empty()) {
        emit_validation_error(
            diagnostics,
            "durable store import decision review summary workflow_canonical_name "
            "must not be empty");
    }
    ...
    if (summary.run_id.has_value() && summary.run_id->empty()) {
        emit_validation_error(
            diagnostics,
            "durable store import decision review summary run_id must not be empty "
            "when present");
    }
    ...
    if (summary.export_package_identity.empty()) {
        emit_validation_error(
            diagnostics,
            "durable store import decision review summary export_package_identity "
            "must not be empty");
    }
    ...
    if (summary.accepted_for_future_execution && summary.decision_blocker.has_value()) {
        emit_validation_error(diagnostics,
                              "durable store import decision review summary cannot contain "
    ...
    case DecisionStatus::Accepted:
        ...
        break;
    case DecisionStatus::Blocked:
        if (summary.next_action != DecisionReviewNextActionKind::ResolveRequiredAdapterCapability) {
            emit_validation_error(diagnostics,
                                  "durable store import decision review summary Blocked decision "
    ...
    case DecisionStatus::Deferred:
        if (summary.next_action !=
            ...
            emit_validation_error(
                ...
                "next_action preserve_partial_durable_store_import_decision");
        }
        if (!summary.decision_blocker.has_value()) {
            emit_validation_error(diagnostics,
                                  "durable store import decision review summary Deferred decision "
                                  "requires decision_blocker");
        }
        break;
    case DecisionStatus::Rejected:
        if (summary.next_action !=
            ...
            emit_validation_error(
                ...
                "next_action investigate_durable_store_import_decision_rejection");
        }
        if (!summary.decision_blocker.has_value()) {
            emit_validation_error(diagnostics,
                                  "durable store import decision review summary Rejected decision "
                                  "requires decision_blocker");
        }
        break;
    }
...
DecisionReviewSummaryResult build_decision_review_summary(const Decision &decision) {
