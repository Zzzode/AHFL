...
void validate_adapter_blocker(const RequestBlocker &blocker, DiagnosticBag &diagnostics) {
    ...
    if (blocker.logical_artifact_name.has_value() && blocker.logical_artifact_name->empty()) {
        emit_validation_error(diagnostics,
                              "durable store import review summary adapter_blocker "
...
[[nodiscard]] std::string adapter_boundary_summary_for_request(const Request &request) {
    ...
    case RequestBoundaryKind::AdapterContractConsumable:
        return "adapter-contract-consumable request shape available; durable mutation ABI still "
               "not promised";
    }
...
[[nodiscard]] std::string request_preview_for_request(const Request &request) {
    switch (request.request_status) {
    case RequestStatus::ReadyForAdapter:
        return "durable store import request '" + request.durable_store_import_request_identity +
    ...
    case RequestStatus::TerminalCompleted:
        return "workflow already completed; durable store import request is retained for archival "
               "review";
    case RequestStatus::TerminalFailed:
        if (request.workflow_failure_summary.has_value() &&
            request.workflow_failure_summary->node_name.has_value()) {
            return "workflow failed at node '" + *request.workflow_failure_summary->node_name +
                   "'; durable store import request handoff is closed";
        }
        return "workflow failed; durable store import request handoff is closed";
    case RequestStatus::TerminalPartial:
        return "partial workflow is retained as local durable store import request without adapter "
               "handoff";
    }
...
[[nodiscard]] std::string next_step_recommendation_for_request(const Request &request) {
    switch (request.request_status) {
    case RequestStatus::ReadyForAdapter:
        return "handoff current durable store import request before real adapter implementation "
               "work";
    case RequestStatus::Blocked:
        if (request.next_required_adapter_artifact_kind.has_value()) {
    ...
    case RequestStatus::TerminalPartial:
        return "preserve partial durable store import request for inspection; do not advertise "
               "adapter handoff";
    }
...
requested_artifact_preview_for_request(const Request &request) {
...
ReviewSummaryValidationResult validate_review_summary(const ReviewSummary &summary) {
    ReviewSummaryValidationResult result;
    auto &diagnostics = result.diagnostics;

    if (summary.format_version != kReviewSummaryFormatVersion) {
        emit_validation_error(diagnostics,
                              "durable store import review summary format_version must be '" +
    ...
    if (summary.source_durable_store_import_request_format_version != kRequestFormatVersion) {
        emit_validation_error(diagnostics,
                              "durable store import review summary "
    ...
    if (summary.session_id.empty()) {
        emit_validation_error(diagnostics,
                              "durable store import review summary session_id must not be empty");
    }
    ...
    if (summary.input_fixture.empty()) {
        emit_validation_error(
            diagnostics, "durable store import review summary input_fixture must not be empty");
    }
    ...
    if (summary.store_import_candidate_identity.empty()) {
        emit_validation_error(diagnostics,
                              "durable store import review summary store_import_candidate_identity "
                              "must not be empty");
    }
    ...
    if (summary.requested_artifact_entry_count != summary.requested_artifact_preview.size()) {
        emit_validation_error(
            diagnostics,
            "durable store import review summary requested_artifact_entry_count must "
    ...
    for (const auto &entry : summary.requested_artifact_preview) {
        if (entry.empty()) {
            emit_validation_error(
                diagnostics,
                "durable store import review summary requested_artifact_preview must "
    ...
    if (summary.next_required_adapter_artifact_kind.has_value() && summary.adapter_ready) {
        emit_validation_error(diagnostics,
                              "durable store import review summary cannot contain "
    ...
    if (summary.adapter_ready && summary.adapter_blocker.has_value()) {
        emit_validation_error(
            diagnostics,
            "durable store import review summary cannot contain adapter_blocker when "
    ...
    if (summary.request_preview.empty()) {
        emit_validation_error(
            diagnostics, "durable store import review summary request_preview must not be empty");
    }
    ...
    case RequestStatus::ReadyForAdapter:
        ...
        break;
    case RequestStatus::Blocked:
        if (summary.next_action != ReviewNextActionKind::AwaitAdapterReadiness) {
            emit_validation_error(
                diagnostics,
                "durable store import review summary Blocked request_status requires "
                "next_action await_adapter_readiness");
        }
        if (summary.adapter_ready) {
            emit_validation_error(diagnostics,
                                  "durable store import review summary Blocked request_status "
    ...
    case RequestStatus::TerminalCompleted:
        if (summary.next_action != ReviewNextActionKind::ArchiveCompletedDurableStoreImportState) {
            emit_validation_error(
                ...
                "next_action archive_completed_durable_store_import_state");
        }
        if (!summary.adapter_ready) {
            emit_validation_error(diagnostics,
                                  "durable store import review summary TerminalCompleted "
                                  "request_status requires adapter_ready");
        }
        if (summary.adapter_blocker.has_value()) {
            emit_validation_error(diagnostics,
                                  "durable store import review summary TerminalCompleted "
                                  "request_status cannot contain adapter_blocker");
        }
        if (summary.next_required_adapter_artifact_kind.has_value()) {
            emit_validation_error(
                diagnostics,
                "durable store import review summary TerminalCompleted request_status cannot "
                "contain next_required_adapter_artifact_kind");
        }
        break;
    case RequestStatus::TerminalFailed:
        if (summary.next_action != ReviewNextActionKind::InvestigateDurableStoreImportFailure) {
            emit_validation_error(
                ...
                "next_action investigate_durable_store_import_failure");
        }
        if (!summary.adapter_blocker.has_value()) {
            emit_validation_error(
                diagnostics,
                "durable store import review summary TerminalFailed request_status "
    ...
    case RequestStatus::TerminalPartial:
        if (summary.next_action != ReviewNextActionKind::PreservePartialDurableStoreImportState) {
            emit_validation_error(
                ...
                "next_action preserve_partial_durable_store_import_state");
        }
        if (!summary.adapter_blocker.has_value()) {
            emit_validation_error(
                diagnostics,
                "durable store import review summary TerminalPartial request_status "
                "requires adapter_blocker");
        }
        break;
    }
...
ReviewSummaryResult build_review_summary(const Request &request) {
    ...
    ReviewSummary summary{
        ...
        .next_required_adapter_artifact_kind = request.next_required_adapter_artifact_kind,
