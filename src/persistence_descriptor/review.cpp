...
void validate_persistence_blocker(const PersistenceBlocker &blocker, DiagnosticBag &diagnostics) {
    if (blocker.message.empty()) {
        diagnostics.error("persistence review summary persistence_blocker message must not be empty");
    }

    if (blocker.node_name.has_value() && blocker.node_name->empty()) {
        diagnostics.error(
            "persistence review summary persistence_blocker node_name must not be empty");
    }
}
...
validate_persistence_review_summary(const PersistenceReviewSummary &summary) {
    ...
    if (summary.format_version != kPersistenceReviewSummaryFormatVersion) {
        diagnostics.error("persistence review summary format_version must be '" +
    ...
    if (summary.source_persistence_descriptor_format_version !=
        ...
        diagnostics.error(
            "persistence review summary source_persistence_descriptor_format_version must be '" +
    ...
    if (summary.workflow_canonical_name.empty()) {
        diagnostics.error("persistence review summary workflow_canonical_name must not be empty");
    }

    if (summary.session_id.empty()) {
        diagnostics.error("persistence review summary session_id must not be empty");
    }

    if (summary.run_id.has_value() && summary.run_id->empty()) {
        diagnostics.error("persistence review summary run_id must not be empty when present");
    }

    if (summary.input_fixture.empty()) {
        diagnostics.error("persistence review summary input_fixture must not be empty");
    }

    if (summary.planned_durable_identity.empty()) {
        diagnostics.error("persistence review summary planned_durable_identity must not be empty");
    }
    ...
    if (summary.exportable_prefix_size != summary.exportable_prefix.size()) {
        diagnostics.error(
            "persistence review summary exportable_prefix_size must match exportable_prefix length");
    }

    if (summary.next_export_candidate_node_name.has_value() &&
        summary.next_export_candidate_node_name->empty()) {
        diagnostics.error(
            "persistence review summary next_export_candidate_node_name must not be empty");
    }
    ...
    if (summary.export_ready && summary.persistence_blocker.has_value()) {
        diagnostics.error(
            "persistence review summary cannot contain persistence_blocker when export_ready is true");
    }

    if (summary.store_boundary_summary.empty()) {
        diagnostics.error("persistence review summary store_boundary_summary must not be empty");
    }

    if (summary.export_preview.empty()) {
        diagnostics.error("persistence review summary export_preview must not be empty");
    }

    if (summary.next_step_recommendation.empty()) {
        diagnostics.error(
            "persistence review summary next_step_recommendation must not be empty");
    }
    ...
    case PersistenceDescriptorStatus::ReadyToExport:
        if (summary.next_action !=
            ...
            diagnostics.error(
                "persistence review summary ReadyToExport persistence_status requires next_action export_persistence_handoff");
        }
        if (!summary.export_ready) {
            diagnostics.error(
                "persistence review summary ReadyToExport persistence_status requires export_ready");
        }
        if (!summary.next_export_candidate_node_name.has_value()) {
            diagnostics.error(
                "persistence review summary ReadyToExport persistence_status requires next_export_candidate_node_name");
        }
        break;
    case PersistenceDescriptorStatus::Blocked:
        if (summary.next_action !=
            ...
            diagnostics.error(
                "persistence review summary Blocked persistence_status requires next_action await_persistence_readiness");
        }
        if (summary.export_ready) {
            diagnostics.error(
        ...
        if (!summary.persistence_blocker.has_value()) {
            diagnostics.error(
                "persistence review summary Blocked persistence_status requires persistence_blocker");
        }
        break;
    case PersistenceDescriptorStatus::TerminalCompleted:
        if (summary.next_action != PersistenceReviewNextActionKind::WorkflowCompleted) {
            diagnostics.error(
                "persistence review summary TerminalCompleted persistence_status requires next_action workflow_completed");
        }
        if (summary.persistence_blocker.has_value()) {
            diagnostics.error(
                "persistence review summary TerminalCompleted persistence_status cannot contain persistence_blocker");
        }
        if (summary.next_export_candidate_node_name.has_value()) {
            diagnostics.error(
                "persistence review summary TerminalCompleted persistence_status cannot contain next_export_candidate_node_name");
        }
        break;
    case PersistenceDescriptorStatus::TerminalFailed:
        if (summary.next_action != PersistenceReviewNextActionKind::InvestigateFailure) {
            diagnostics.error(
                "persistence review summary TerminalFailed persistence_status requires next_action investigate_failure");
        }
        if (!summary.persistence_blocker.has_value()) {
            diagnostics.error(
                "persistence review summary TerminalFailed persistence_status requires persistence_blocker");
        }
        break;
    case PersistenceDescriptorStatus::TerminalPartial:
        if (summary.next_action != PersistenceReviewNextActionKind::PreservePartialState) {
            diagnostics.error(
                "persistence review summary TerminalPartial persistence_status requires next_action preserve_partial_state");
        }
        if (!summary.persistence_blocker.has_value()) {
            diagnostics.error(
                "persistence review summary TerminalPartial persistence_status requires persistence_blocker");
        }
        break;
    }
    ...
    if (summary.workflow_status == runtime_session::WorkflowSessionStatus::Completed &&
        ...
        diagnostics.error(
            "persistence review summary completed workflow_status requires TerminalCompleted persistence_status");
    }
    ...
    if (summary.workflow_status == runtime_session::WorkflowSessionStatus::Failed &&
        ...
        diagnostics.error(
            "persistence review summary failed workflow_status requires TerminalFailed persistence_status");
    }
    ...
    if (summary.checkpoint_status ==
        ...
        summary.persistence_status != PersistenceDescriptorStatus::TerminalCompleted) {
        diagnostics.error(
            "persistence review summary TerminalCompleted checkpoint_status requires TerminalCompleted persistence_status");
    }
    ...
    if (summary.checkpoint_status ==
        ...
        summary.persistence_status != PersistenceDescriptorStatus::TerminalFailed) {
        diagnostics.error(
            "persistence review summary TerminalFailed checkpoint_status requires TerminalFailed persistence_status");
    }
    ...
    if (summary.checkpoint_status ==
        ...
        diagnostics.error(
            "persistence review summary TerminalPartial checkpoint_status requires TerminalPartial persistence_status");
    }
    ...
    if (summary.workflow_status == runtime_session::WorkflowSessionStatus::Partial &&
        ...
        summary.persistence_status != PersistenceDescriptorStatus::TerminalPartial) {
        diagnostics.error(
            "persistence review summary partial workflow_status must map to ReadyToExport, Blocked, or TerminalPartial persistence_status");
    }
