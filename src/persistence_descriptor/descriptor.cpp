...
void validate_failure_summary(const runtime_session::RuntimeFailureSummary &summary,
                              DiagnosticBag &diagnostics) {
    validation::validate_failure_summary_field(summary, diagnostics, "persistence descriptor");
}
...
to_persistence_blocker_kind(checkpoint_record::CheckpointResumeBlockerKind kind) {
    switch (kind) {
...
validate_persistence_descriptor(const CheckpointPersistenceDescriptor &descriptor) {
    ...
    if (descriptor.format_version != kPersistenceDescriptorFormatVersion) {
        diagnostics.error()
            .message("persistence descriptor format_version must be '" +
    ...
    if (descriptor.source_execution_plan_format_version != handoff::kExecutionPlanFormatVersion) {
        diagnostics.error()
            .message("persistence descriptor source_execution_plan_format_version must be '" +
    ...
    if (descriptor.source_runtime_session_format_version !=
        ...
        diagnostics.error()
            .message("persistence descriptor source_runtime_session_format_version must be '" +
    ...
    if (descriptor.source_execution_journal_format_version !=
        ...
        diagnostics.error()
            .message("persistence descriptor source_execution_journal_format_version must be '" +
    ...
    if (descriptor.source_replay_view_format_version != replay_view::kReplayViewFormatVersion) {
        diagnostics.error()
            .message("persistence descriptor source_replay_view_format_version must be '" +
    ...
    if (descriptor.source_scheduler_snapshot_format_version !=
        ...
        diagnostics.error()
            .message("persistence descriptor source_scheduler_snapshot_format_version must be '" +
    ...
    if (descriptor.source_checkpoint_record_format_version !=
        ...
        diagnostics.error()
            .message("persistence descriptor source_checkpoint_record_format_version must be '" +
    ...
    if (descriptor.workflow_canonical_name.empty()) {
        diagnostics.error()
            .message("persistence descriptor workflow_canonical_name must not be empty")
            .emit();
    }
    ...
    if (descriptor.input_fixture.empty()) {
        diagnostics.error()
            .message("persistence descriptor input_fixture must not be empty")
            .emit();
    }

    if (descriptor.planned_durable_identity.empty()) {
        diagnostics.error()
            .message("persistence descriptor planned_durable_identity must not be empty")
            .emit();
    }
    ...
    for (const auto &node_name : descriptor.execution_order) {
        if (node_name.empty()) {
            diagnostics.error()
                .message("persistence descriptor execution_order contains empty node name")
                .emit();
            continue;
        }
        ...
        if (!execution_nodes.insert(node_name).second) {
            diagnostics.error()
                .message("persistence descriptor execution_order contains duplicate node '" +
                         node_name + "'")
                .emit();
        }
    }
    ...
    if (descriptor.cursor.exportable_prefix_size != descriptor.cursor.exportable_prefix.size()) {
        diagnostics.error()
            .message("persistence descriptor cursor exportable_prefix_size must match "
    ...
    if (descriptor.cursor.exportable_prefix.size() > descriptor.execution_order.size()) {
        diagnostics.error()
            .message("persistence descriptor cursor exportable_prefix cannot be longer than "
                     "execution_order")
            .emit();
    }
    ...
    for (std::size_t index = 0; index < descriptor.cursor.exportable_prefix.size(); ++index) {
        if (descriptor.cursor.exportable_prefix[index] != descriptor.execution_order[index]) {
            diagnostics.error()
                .message("persistence descriptor cursor exportable_prefix must be a prefix of "
                         "execution_order")
                .emit();
            break;
        }
    ...
    if (descriptor.cursor.next_export_candidate_node_name.has_value()) {
        if (descriptor.cursor.next_export_candidate_node_name->empty()) {
            diagnostics.error()
                .message("persistence descriptor cursor next_export_candidate_node_name must not "
                         "be empty")
                .emit();
        } else if (!execution_nodes.contains(*descriptor.cursor.next_export_candidate_node_name)) {
            diagnostics.error()
                .message("persistence descriptor cursor next_export_candidate_node_name '" +
    ...
    if (descriptor.cursor.export_ready && descriptor.persistence_blocker.has_value()) {
        diagnostics.error()
            .message("persistence descriptor cannot contain persistence_blocker when cursor "
    ...
    if (!descriptor.cursor.export_ready &&
        ...
        diagnostics.error()
            .message("persistence descriptor must contain persistence_blocker when cursor "
    ...
    if (descriptor.persistence_blocker.has_value()) {
        if (descriptor.persistence_blocker->message.empty()) {
            diagnostics.error()
                .message("persistence descriptor persistence_blocker message must not be empty")
                .emit();
        }
        ...
        if (descriptor.persistence_blocker->node_name.has_value() &&
            ...
            diagnostics.error()
                .message("persistence descriptor persistence_blocker node_name must not be empty")
                .emit();
        }
    }
    ...
    if (descriptor.persistence_status == PersistenceDescriptorStatus::ReadyToExport &&
        ...
        diagnostics.error()
            .message("persistence descriptor ReadyToExport status requires cursor export_ready")
            .emit();
    }
    ...
    if (descriptor.persistence_status == PersistenceDescriptorStatus::ReadyToExport &&
        ...
        diagnostics.error()
            .message("persistence descriptor ReadyToExport status requires "
    ...
    if (descriptor.persistence_status == PersistenceDescriptorStatus::ReadyToExport &&
        ...
        diagnostics.error()
            .message("persistence descriptor ReadyToExport status requires ReadyToPersist "
                     "checkpoint_status")
            .emit();
    }
    ...
    if (descriptor.export_basis_kind == PersistenceBasisKind::StoreAdjacent &&
        ...
        descriptor.persistence_status != PersistenceDescriptorStatus::TerminalCompleted) {
        diagnostics.error()
            .message("persistence descriptor store-adjacent basis requires ready or completed "
                     "persistence_status")
            .emit();
    }
    ...
    if (descriptor.persistence_status == PersistenceDescriptorStatus::TerminalCompleted &&
        descriptor.cursor.next_export_candidate_node_name.has_value()) {
        diagnostics.error()
            .message("persistence descriptor TerminalCompleted status cannot have "
                     "next_export_candidate_node_name")
            .emit();
    }
    ...
    if ((descriptor.persistence_status == PersistenceDescriptorStatus::TerminalFailed ||
        ...
        !descriptor.persistence_blocker.has_value()) {
        diagnostics.error()
            .message("persistence descriptor terminal blocked status requires persistence_blocker")
            .emit();
    }
    ...
    if ((descriptor.persistence_status == PersistenceDescriptorStatus::TerminalFailed ||
        ...
        descriptor.cursor.export_ready) {
        diagnostics.error()
            .message("persistence descriptor terminal blocked status cannot be export_ready")
            .emit();
    }
    ...
    if ((descriptor.persistence_status == PersistenceDescriptorStatus::TerminalFailed ||
        ...
        descriptor.cursor.next_export_candidate_node_name.has_value()) {
        diagnostics.error()
            .message("persistence descriptor terminal blocked status cannot have "
                     "next_export_candidate_node_name")
            .emit();
    }
    ...
    if (descriptor.workflow_status == runtime_session::WorkflowSessionStatus::Completed &&
        descriptor.persistence_status != PersistenceDescriptorStatus::TerminalCompleted) {
        diagnostics.error()
            .message("persistence descriptor completed workflow_status requires TerminalCompleted "
                     "persistence_status")
            .emit();
    }
    ...
    if (descriptor.workflow_status == runtime_session::WorkflowSessionStatus::Failed &&
        ...
        diagnostics.error()
            .message("persistence descriptor failed workflow_status requires TerminalFailed "
                     "persistence_status")
            .emit();
    }
    ...
    if (descriptor.checkpoint_status ==
        ...
        descriptor.persistence_status != PersistenceDescriptorStatus::TerminalCompleted) {
        diagnostics.error()
            .message("persistence descriptor TerminalCompleted checkpoint_status requires "
    ...
    if (descriptor.checkpoint_status == checkpoint_record::CheckpointRecordStatus::TerminalFailed &&
        descriptor.persistence_status != PersistenceDescriptorStatus::TerminalFailed) {
        diagnostics.error()
            .message("persistence descriptor TerminalFailed checkpoint_status requires "
    ...
    if (descriptor.checkpoint_status ==
        ...
        descriptor.persistence_status != PersistenceDescriptorStatus::TerminalPartial) {
        diagnostics.error()
            .message("persistence descriptor TerminalPartial checkpoint_status requires "
    ...
    if (descriptor.workflow_status == runtime_session::WorkflowSessionStatus::Partial &&
        ...
        descriptor.persistence_status != PersistenceDescriptorStatus::TerminalPartial) {
        diagnostics.error()
            .message("persistence descriptor partial workflow_status must map to ReadyToExport, "
                     "Blocked, or TerminalPartial persistence_status")
            .emit();
    }
...
build_persistence_descriptor(const handoff::ExecutionPlan &plan,
    ...
    if (session.source_execution_plan_format_version != plan.format_version) {
        result.diagnostics.error()
            .message("persistence descriptor bootstrap runtime session "
    ...
    if (journal.source_execution_plan_format_version != plan.format_version) {
        result.diagnostics.error()
            .message("persistence descriptor bootstrap execution journal "
                     "source_execution_plan_format_version does not match execution plan")
            .emit();
    }
    ...
    if (replay.source_execution_plan_format_version != plan.format_version) {
        result.diagnostics.error()
            .message("persistence descriptor bootstrap replay view "
                     "source_execution_plan_format_version does not match execution plan")
            .emit();
    }
    ...
    if (snapshot.source_execution_plan_format_version != plan.format_version) {
        result.diagnostics.error()
            .message("persistence descriptor bootstrap scheduler snapshot "
                     "source_execution_plan_format_version does not match execution plan")
            .emit();
    }
    ...
    if (record.source_execution_plan_format_version != plan.format_version) {
        result.diagnostics.error()
            .message("persistence descriptor bootstrap checkpoint record "
                     "source_execution_plan_format_version does not match execution plan")
            .emit();
    }
    ...
    if (!package_identity_equals(plan.source_package_identity, session.source_package_identity)) {
        result.diagnostics.error()
            .message("persistence descriptor bootstrap runtime session source_package_identity "
    ...
    if (!package_identity_equals(plan.source_package_identity, journal.source_package_identity)) {
        result.diagnostics.error()
            .message("persistence descriptor bootstrap execution journal source_package_identity "
                     "does not match execution plan")
            .emit();
    }
    ...
    if (!package_identity_equals(plan.source_package_identity, replay.source_package_identity)) {
        result.diagnostics.error()
            .message("persistence descriptor bootstrap replay view source_package_identity does "
                     "not match execution plan")
            .emit();
    }

    if (!package_identity_equals(plan.source_package_identity, snapshot.source_package_identity)) {
        result.diagnostics.error()
            .message("persistence descriptor bootstrap scheduler snapshot source_package_identity "
                     "does not match execution plan")
            .emit();
    }
    ...
    if (!package_identity_equals(plan.source_package_identity, record.source_package_identity)) {
        result.diagnostics.error()
            .message("persistence descriptor bootstrap checkpoint record source_package_identity "
                     "does not match execution plan")
            .emit();
    }
    ...
    if (session.workflow_canonical_name != journal.workflow_canonical_name) {
        result.diagnostics.error()
            .message("persistence descriptor bootstrap execution journal workflow_canonical_name "
    ...
    if (session.workflow_canonical_name != replay.workflow_canonical_name) {
        result.diagnostics.error()
            .message("persistence descriptor bootstrap replay view workflow_canonical_name does "
    ...
    if (session.workflow_canonical_name != snapshot.workflow_canonical_name) {
        result.diagnostics.error()
            .message("persistence descriptor bootstrap scheduler snapshot workflow_canonical_name "
                     "does not match runtime session")
            .emit();
    }
    ...
    if (session.workflow_canonical_name != record.workflow_canonical_name) {
        result.diagnostics.error()
            .message("persistence descriptor bootstrap checkpoint record workflow_canonical_name "
                     "does not match runtime session")
            .emit();
    }
    ...
    if (session.session_id != journal.session_id) {
        result.diagnostics.error()
            .message("persistence descriptor bootstrap execution journal session_id does not match "
                     "runtime session")
            .emit();
    }
    ...
    if (session.session_id != replay.session_id) {
        result.diagnostics.error()
            .message("persistence descriptor bootstrap replay view session_id does not match "
                     "runtime session")
            .emit();
    }
    ...
    if (session.session_id != snapshot.session_id) {
        result.diagnostics.error()
            .message("persistence descriptor bootstrap scheduler snapshot session_id does not "
    ...
    if (session.session_id != record.session_id) {
        result.diagnostics.error()
            .message("persistence descriptor bootstrap checkpoint record session_id does not match "
                     "runtime session")
            .emit();
    }
    ...
    if (session.run_id != journal.run_id) {
        result.diagnostics.error()
            .message("persistence descriptor bootstrap execution journal run_id does not match "
                     "runtime session")
            .emit();
    }
    ...
    if (session.run_id != replay.run_id) {
        result.diagnostics.error()
            .message("persistence descriptor bootstrap replay view run_id does not match runtime "
                     "session")
            .emit();
    }
    ...
    if (session.run_id != snapshot.run_id) {
        result.diagnostics.error()
            .message("persistence descriptor bootstrap scheduler snapshot run_id does not match "
                     "runtime session")
            .emit();
    }
    ...
    if (session.run_id != record.run_id) {
        result.diagnostics.error()
            .message("persistence descriptor bootstrap checkpoint record run_id does not match "
                     "runtime session")
            .emit();
    }
    ...
    if (session.input_fixture != replay.input_fixture) {
        result.diagnostics.error()
            .message("persistence descriptor bootstrap replay view input_fixture does not match "
                     "runtime session")
            .emit();
    }
    ...
    if (session.input_fixture != snapshot.input_fixture) {
        result.diagnostics.error()
            .message("persistence descriptor bootstrap scheduler snapshot input_fixture does not "
                     "match runtime session")
            .emit();
    }
    ...
    if (session.input_fixture != record.input_fixture) {
        result.diagnostics.error()
            .message("persistence descriptor bootstrap checkpoint record input_fixture does not "
                     "match runtime session")
            .emit();
    }
    ...
    if (session.workflow_status != replay.workflow_status) {
        result.diagnostics.error()
            .message("persistence descriptor bootstrap replay view workflow_status does not match "
                     "runtime session")
            .emit();
    }
    ...
    if (session.workflow_status != snapshot.workflow_status) {
        result.diagnostics.error()
            .message("persistence descriptor bootstrap scheduler snapshot workflow_status does not "
                     "match runtime session")
            .emit();
    }
    ...
    if (session.workflow_status != record.workflow_status) {
        result.diagnostics.error()
            .message("persistence descriptor bootstrap checkpoint record workflow_status does not "
                     "match runtime session")
            .emit();
    }
    ...
    if (!failure_summary_equals(session.failure_summary, replay.workflow_failure_summary)) {
        result.diagnostics.error()
            .message("persistence descriptor bootstrap replay view workflow_failure_summary does "
                     "not match runtime session")
            .emit();
    }
    ...
    if (!failure_summary_equals(session.failure_summary, snapshot.workflow_failure_summary)) {
        result.diagnostics.error()
            .message("persistence descriptor bootstrap scheduler snapshot workflow_failure_summary "
                     "does not match runtime session")
            .emit();
    }
    ...
    if (!failure_summary_equals(session.failure_summary, record.workflow_failure_summary)) {
        result.diagnostics.error()
            .message("persistence descriptor bootstrap checkpoint record workflow_failure_summary "
                     "does not match runtime session")
            .emit();
    }
    ...
    if (!replay.consistency.plan_matches_session || !replay.consistency.session_matches_journal ||
        ...
        result.diagnostics.error()
            .message("persistence descriptor bootstrap replay view consistency must hold before "
                     "building persistence descriptor")
            .emit();
    }
    ...
    if (workflow == nullptr) {
        result.diagnostics.error()
            .message("persistence descriptor bootstrap workflow '" +
                     session.workflow_canonical_name + "' does not exist in execution plan")
            .emit();
    }
    ...
    if (!vector_equals(snapshot.execution_order, plan_execution_order)) {
        result.diagnostics.error()
            .message("persistence descriptor bootstrap scheduler snapshot execution_order does not "
                     "match execution plan workflow order")
            .emit();
    }

    if (!vector_equals(replay.execution_order, session.execution_order)) {
        result.diagnostics.error()
            .message("persistence descriptor bootstrap replay view execution_order does not match "
                     "runtime session")
            .emit();
    }
    ...
    if (!is_prefix(replay.execution_order, snapshot.execution_order)) {
        result.diagnostics.error()
            .message("persistence descriptor bootstrap replay view execution_order must be a "
                     "prefix of scheduler snapshot execution_order")
            .emit();
    }

    if (!is_prefix(snapshot.cursor.completed_prefix, replay.execution_order)) {
        result.diagnostics.error()
            .message("persistence descriptor bootstrap scheduler snapshot completed_prefix must be "
                     "a prefix of replay view execution_order")
            .emit();
    }

    if (record.snapshot_status != snapshot.snapshot_status) {
        result.diagnostics.error()
            .message("persistence descriptor bootstrap checkpoint record snapshot_status does not "
    ...
    if (!vector_equals(record.execution_order, snapshot.execution_order)) {
        result.diagnostics.error()
            .message("persistence descriptor bootstrap checkpoint record execution_order does not "
                     "match scheduler snapshot")
            .emit();
    }
    ...
    if (!vector_equals(record.cursor.persistable_prefix, snapshot.cursor.completed_prefix)) {
        result.diagnostics.error()
            .message("persistence descriptor bootstrap checkpoint record persistable_prefix does "
                     "not match scheduler snapshot completed_prefix")
            .emit();
    }
    ...
    if (record.checkpoint_friendly_source != snapshot.cursor.checkpoint_friendly) {
        result.diagnostics.error()
            .message("persistence descriptor bootstrap checkpoint record "
                     "checkpoint_friendly_source does not match scheduler snapshot")
            .emit();
    }
