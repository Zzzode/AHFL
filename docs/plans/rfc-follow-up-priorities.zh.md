...
1. RFC 0003 后续 pattern usefulness matrix。
   - 状态：RFC 0011 已新增；`PatternUsefulnessContext` / finite constructor matrix 第一条基础设施切片已落库并有单元测试。
   ...
   - 剩余核心工作：source pattern → typed pattern HIR lowering、替换 RFC 0003 ad-hoc checker、witness diagnostics、if-let/optional narrowing flow integration、LSP quick fix。
