...
1. RFC 0003 后续 pattern usefulness matrix。
   ...
   - 剩余核心工作：future 更深 completion / code-action editing UX 继续消费 typed pattern HIR；range usefulness core 已落库，但 parser/frontend/typechecker/typed-HIR/LSP range pattern surface 仍需等待 range pattern syntax 被接受后再接入，不预留 placeholder diagnostic code。
