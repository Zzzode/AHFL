...
| RFC 0004: Native gRPC Transport | `draft` | 未完成。现有 runtime gRPC 路径是 `grpc_json_transcoding`，不等价于 native gRPC/Protobuf transport；当前 registry 也不做 transport scheme dispatch。RFC0004 已改为 fail-closed 默认、显式 fallback、proto contract 治理和四路 benchmark baseline（`http_json` / `http2_json_optimized` / `grpc_json_transcoding` / `native_grpc`）；native gRPC machine gate 已禁止在 Go 决策前引入 build flag、runtime config marker、C++ dependency wiring、proto service contract 或 `native-grpc` implementation artifact。 | P3：先做 runtime owner decision gate、HTTP/2 JSON optimized baseline、四路 benchmark、三平台构建和依赖策略评估，再决定是否进入 accepted/implementing。 |
...
| RFC 0007: LSP Workspace Navigation Index | `stabilized` | v1 完成。semantic graph 与 navigation index 分离、primitive home、impl/reference/navigation 索引、stable fingerprint、rename、CodeLens/reference/implementation v1、open overlay invalidation、process-local previous-index fact remap、stale completeness reuse guard 和 multi-root profile isolation 均已落库。 | 只保留 UX / performance polish；public API compatibility gate 已转入 RFC0010。 |
...
| RFC 0011: Pattern Usefulness Matrix | `stabilized` | 完成。RFC0003 后续 nested/literal/range/payload destructuring usefulness matrix 已拆成独立 RFC；matrix core、`TypedProgram::patterns` 首批基础设施、match consumer typed-row 迁移、if-let statement typed pattern root、if-let unreachable-else matrix diagnostic、if-let narrowing typed-pattern consumer、if-let 通用 `PatternSyntax` 语法/AST 迁移、match-arm narrowing typed-pattern consumer、or-pattern binding equivalence、LSP rendered witness arm quick fix v1、结构化 witness diagnostic payload、missing-pattern 多行 witness arm quick fix、unreachable match arm quick fix、if-let unreachable else quick fix、redundant or-pattern branch related notes、redundant or-pattern branch quick fix（含多行 struct payload destructuring branch）、invalid Int range quick fix、struct variant missing-field quick fix、struct variant unexpected-field quick fix、struct variant duplicate-field quick fix、struct variant rest-pattern-aware field completion textEdit、wildcard-pattern replacement completion textEdit、binding-pattern replacement completion textEdit、struct payload child-pattern priority completion、typed-pattern-driven pattern binding hover v1、typed-pattern-driven pattern binding local navigation/rename v1、typed-pattern-driven local binding documentHighlight v1、typed-pattern-driven pattern-aware selectionRange v1、typed-pattern-driven enum variant/struct payload field completion v1、typed-pattern-driven Bool literal pattern completion v1、typed-pattern-driven bounded Int pattern completion v1、typed-pattern-driven open primitive pattern completion v1、typed-pattern-driven bounded String singleton completion v1、typed-pattern-driven bounded String singleton pattern matrix v1、typed-pattern-driven enum pattern payload signatureHelp v1、client-gated enum pattern payload destructuring snippets、client-gated struct payload field completion snippets、nested enum payload snippet choices、primitive payload snippet choices、RFC0011 representative release evidence、非 Bool open literal usefulness、decoded String literal / numeric Float literal constructor identity、Int range pattern source surface v1、signed Int range bounds、bounded Int matrix infrastructure、非枚举 bounded Int interval analysis、source-level `Int(min,max)` 初始切片、大型嵌套 bounded Int product symbolic analysis、bounded Int signed literal singleton inference、bounded Int arithmetic range inference（含 unary negation、非零 `/`、finite variable-divisor 精确 `%`、divisor-dominates oversized exact `%`、quotient-partition large-domain exact `%` 和预算/溢出保守回退）、bounded String interval validation / literal singleton inference、bounded String concatenation range inference、Decimal multiplication product-scale semantics、显式 Decimal division target-scale/rounding API 和当前 pattern diagnostic taxonomy 已落库，且 spec/reference/release evidence 已满足稳定化出口。 | 只保留 post-v1 UX / refinement 扩展：更深 destructuring code-action、未来 Float refinement semantics、非 singleton String pattern domain 和 LSP editing polish 需另行收口。 |
...
### P2：post-v1 refinement backlog

1. RFC 0011 post-v1 pattern usefulness extensions。
   - 状态：RFC 0011 已新增；`PatternUsefulnessContext` / finite constructor matrix 第一条基础设施切片已落库并有单元测试；RFC 0003 `match_exhaustiveness` 已通过 matrix core 执行 enum coverage，并已接入 typed enum-payload lowering 覆盖 nested enum payload witness、Bool payload literal witness、struct payload 字段名 witness rendering、non-Bool open literal 默认 witness、decoded String literal constructor identity、numeric Float literal constructor identity、Int range source pattern v1、signed Int range bounds、bounded Int matrix infrastructure、非枚举 bounded Int interval analysis、source-level `Int(min,max)` 初始切片、大型嵌套 bounded Int product symbolic analysis、bounded Int signed literal singleton inference、bounded Int arithmetic range inference（含 unary negation、非零 `/`、finite variable-divisor 精确 `%`、divisor-dominates oversized exact `%`、quotient-partition large-domain exact `%` 和预算/溢出保守回退）、bounded String interval validation / literal singleton inference、bounded String concatenation range inference、bounded `String(0,0)` singleton pattern matrix、Decimal multiplication product-scale semantics、显式 Decimal division target-scale/rounding API 和 redundant or-branch warning related notes；`TypedProgram::patterns` 已记录 `match` pattern 的 typed fact flat store 并覆盖 JSON round-trip；常规 `match` exhaustiveness 已迁移为消费 typed pattern root rows；`if let` statement 已记录 typed pattern root index 并覆盖 serialization/monomorphization remap；`if let` unreachable-else warning 已通过 typed-row matrix consumer 发出；`if let` flow narrowing、payload binding introduction 和 match-arm flow narrowing 已改为消费 typed pattern root；`if let` 源语法、AST、formatter、semantic tokens、IR lowering 和 typechecker 已迁移到通用 `PatternSyntax`，旧 `IfLetPatternSyntax` 路径已删除；or-pattern binding equivalence 已按 pattern-local binding set 落库；当前 pattern diagnostic taxonomy 已与 shipped `typecheck.MATCH_*` / `typecheck.MATCH_OR_PATTERN_BINDING_MISMATCH` / `typecheck.UNREACHABLE_IF_LET_ELSE` / `typecheck.INVALID_RANGE_PATTERN` code 对齐；LSP `MATCH_MISSING_PATTERNS` quick fix 只消费结构化 `Diagnostic.data["missing_witnesses"]`，不再从 rendered message 反解析语义或插入 wildcard fallback，并已支持 source-safe 多行 witness arm 插入；`MATCH_UNREACHABLE_ARM` 已提供 source-safe arm 删除 quick fix，覆盖单行 arm 和多行 struct payload destructuring arm；`UNREACHABLE_IF_LET_ELSE` 已提供 source-safe 删除不可达 else branch quick fix；`MATCH_REDUNDANT_PATTERN` 已提供 source-safe 删除 redundant or-pattern branch quick fix，覆盖单行 branch 和多行 struct payload destructuring branch；`INVALID_RANGE_PATTERN` 已提供结构化 source-safe bounds swap quick fix；`MISSING_VARIANT_FIELD` 已提供结构化 data 和 source-safe struct variant pattern missing-field insertion quick fix；`UNEXPECTED_VARIANT_FIELD` 已提供结构化 data 和 source-safe struct variant pattern unexpected-field deletion quick fix；`DUPLICATE_VARIANT_FIELD` 已提供结构化 data 和 source-safe struct variant pattern duplicate-field deletion quick fix，并且 declaration/constructor 场景只暴露 data 不自动编辑；struct variant payload field completion 在光标落在顶层 `..` rest pattern token 时已通过标准 `CompletionItem.textEdit` 替换 rest token，避免裸插入产生无效 source；wildcard pattern completion 已通过标准 `CompletionItem.textEdit` 替换源码 `_` token，且 struct payload 内已有子 pattern 优先按子 pattern `matched_type` 补全，避免 `Data { label: _ }` 被父 payload field completion 遮蔽；pattern binding 声明位点 hover 已通过 `TypedProgram::patterns` 展示 match / if-let binding 名称和类型；pattern binding local navigation/rename 和 documentHighlight 已通过 AST lexical scope 和 typed expression path-root facts 支持 definition/references/prepareRename/rename/highlight，并尊重内层 `let` / lambda parameter shadowing；pattern-aware selectionRange 已把 nested typed pattern ranges 注入选择链；pattern completion 已通过 `TypedProgram::patterns` 的 `matched_type` 在 match / if-let / nested payload pattern 内只返回当前 scrutinee enum 的 variant，在 Bool pattern domain 只返回 `true` / `false` literal pattern，在小型 bounded Int pattern domain 返回域内 literal 和完整 range pattern，在大型 bounded Int pattern domain 只返回完整 range pattern，在 open Int / Float / String primitive pattern domain 返回 source-safe skeleton 并阻止 expression-symbol 泄漏，在非 singleton bounded String pattern domain 保守返回 wildcard，在 `String(0,0)` pattern domain 返回 wildcard 与 `""` literal pattern，并在 struct variant payload braces 内返回尚未出现的 payload field；client 支持 snippet 时，pattern completion 已能返回 tuple/struct enum variant payload destructuring snippet、struct payload field insertion snippet 和 nested enum payload unit-variant choices；pattern payload signatureHelp 已通过 typed variant pattern facts 展示 tuple/struct enum payload 签名和 active parameter。
   - 新增：finite constructor product 触达 witness 枚举上限时，matrix 已切换到 bounded-int interval / symbolic closed-space analyzer；不再用部分 materialized witness 集合证明穷尽性。
   - 新增：LSP pattern completion 已支持 source-safe leaf pattern replacement textEdit，覆盖 literal、signed Int range 和 unit enum variant token；source-safe 单个 identifier binding pattern 也已支持 textEdit 替换；tuple 和 or-pattern 继续保持手动编辑，避免误改复合 pattern 结构。
   - 新增：LSP nested enum payload snippet choices 已扩展到 payload variant skeleton；tuple payload variant 会给出 `Variant(_, ...)` choice，struct payload variant 会给出 `Variant { field: _ }` choice，并对 snippet choice 内部逗号做 LSP grammar 转义。
   ...
   - RFC 0011 v1 已进入 `stabilized`；RFC0003 不再承载完整 matrix 扩展。
   - 剩余扩展工作：future 更深 code-action / editing UX 继续消费 typed pattern HIR；numeric range 后续继续沿用 AST / TypedPattern / IR 一等 numeric range/bounds fact，不能退回字符串反解析；bounded String 当前覆盖 expected-type literal singleton、区间校验、concatenation range inference 和 `String(0,0)` singleton pattern matrix，不声明非 singleton string pattern domain 已进入 matrix；未来 Float refinement semantics 需要单独收口。

2. RFC 0007 post-v1 LSP index polish。
   - 状态：process-local phase II 已完成；当前不再把 RFC0007 作为 active semantic blocker。
   ...
   - stale completeness reuse guard 已落库：symbol/reference facts 只有在 previous fact 与 current fact 的 `FactCompleteness` 一致时才复用，避免另一个 source 的 resolver error 被修复后，未变 source 仍携带旧 Parsed skeleton fact。
...
1. RFC 0004 native gRPC transport。
   ...
   - 先完成 owner decision gate、`http2_json_optimized` baseline、四路 benchmark、三平台构建、依赖策略和 feature flag 策略。
   - 当前 `GrpcJsonTranscoding*` 实现和 runtime capability binding tests 只能证明 JSON transcoding path 成熟，不能算 native gRPC/Protobuf RFC 完成度；RFC0004 为 `draft` 时，仓库禁止引入 native gRPC build flag、runtime config marker、C++ gRPC/Protobuf dependency wiring、native proto service contract 和 `native-grpc` implementation artifact，且会扫描源码与 TOML/CMake/YAML/JSON 等配置文件。
   - 默认 fail-closed；native 失败只有在 unary/idempotent、语义等价有测试证据、capability 显式允许且 observability 完整记录时才允许 fallback。
   - 没有 Go 或 Conditional-Go 决策前不要进入实现；公开 HTTP/JSON provider 默认继续走 JSON / HTTP2 optimized / JSON transcoding 路线。
