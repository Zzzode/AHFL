...
1. RFC 0003 后续 pattern usefulness matrix。
   ...
   - 剩余核心工作：future 更深 code-action / editing UX 继续消费 typed pattern HIR；numeric range 后续继续沿用 AST / TypedPattern / IR 一等 numeric range/bounds fact，不能退回字符串反解析；bounded String 当前只覆盖 expected-type literal singleton、区间校验与 concatenation range inference，不声明完整 string pattern domain 已进入 matrix。
