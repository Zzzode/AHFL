...
1. RFC 0003 后续 pattern usefulness matrix。
   ...
   - 仍需等 struct payload destructuring、更深非 literal refinement propagation、完整精确 modulo interval、Decimal division rounding / target-scale policy、未来 Float refinement semantics 和未来 pattern binding UX 的语言语义稳定后进入 accepted / full implementing。
