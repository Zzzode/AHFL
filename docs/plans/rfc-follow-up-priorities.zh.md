...
1. RFC 0003 后续 pattern usefulness matrix。
   ...
   - 剩余核心工作：future 更深 completion / code-action editing UX 继续消费 typed pattern HIR；Int range、`Int(min,max)` 初始 surface、bounded Int literal singleton inference、bounded Int arithmetic range inference 和 Decimal multiplication product-scale semantics 已落库，后续不要扩展成字符串解析捷径，而应继续沿用 AST / TypedPattern / IR 一等 numeric range/bounds fact。
