...
1. RFC 0003 后续 pattern usefulness matrix。
   ...
   - 新增：LSP pattern completion 已支持 source-safe leaf pattern replacement textEdit，覆盖 literal、signed Int range 和 unit enum variant token；binding、tuple 和 or-pattern 继续保持手动编辑，避免误改复合 pattern 结构。
