...
| 优先级 | 工作 | 当前分类 | 目标 |
|--------|------|----------|------|
| P0 | 受控生产试点闭环 | bounded gate 已实现；长期信心由 live gate 决定 | 当前已有 bounded fault matrix、CI-only single-process hour-scale harness、RSS/allocator 趋势合同、GitHub Actions provenance 和 nightly workflow；本地只检查已下载 evidence，以 `python3 scripts/check-production-confidence-gate.py --require-ready` 判断当前 revision 是否闭环 |
