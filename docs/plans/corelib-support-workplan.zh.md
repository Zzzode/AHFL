...
> **Progress (2026-07-02):** M0 实质完成 — M0-1（`formatted_struct_2spaces.ahfl` 工作树本地分歧对齐 HEAD，4 红→全绿）、M0-2（R1 D1 默认值落地：`/lib/std/` legacy tree 已删除，`/std/` 是唯一 stdlib 源码树）、M0-3（`option_ut` 30/30：25 断言 + 5 负例，已挂 ctest）、M0-4（`AhflInstall.cmake` 加 `install(std/)` 并验证）、M0-5（formatter golden 本地分歧对齐）、M0-6（baked-in std 解析验证）均落地；M0-4 容器 / M0-5 CI 基建部分依赖 M3。本计划已纳入 `docs/README.md` 索引。设计侧（RFC + 4 附件）已定稿并自洽（见 [RFC 0013](../rfcs/0013-corelib-type-system-evolution.zh.md)）。
...
| **IDE / SDK 分发**（VSIX） | `package-vscode-vsix-release.sh` 已打包 `std/` + `std/ahfl.toml` | VS Code extension 通过 `initializationOptions.ahfl.toolchain.bundledSysroot` 提供 bundled fallback；workspace/resource 配置通过 `ahfl.toolchain.sysroot` 发送，禁止再用 LSP process `AHFL_SYSROOT` 注入 | M3（R3-b） |
...
| M0-6 | **stdlib 加载 smoke：干净 cwd + ToolchainProfile/sysroot 场景** | P0 | 完成 | PackageGraph CLI 覆盖 `--sysroot` / `AHFL_SYSROOT` 正例和旧 `AHFL_STDLIB_SEARCH_ROOT` 负例；普通 frontend 覆盖不再从 env、编译期 source dir 或 cwd 自动发现 std | M0-4 |
...
- [x] M0-6 `ahflc check` 经 PackageGraph sysroot 在源文件含 `Option<Int>` 时解析 std（`option_ut` + source-sysroot CLI fixtures 验证）；PackageGraph 入口使用 `--sysroot` / `AHFL_SYSROOT`，并拒绝旧 `AHFL_STDLIB_SEARCH_ROOT` 回退；普通 frontend 不再从旧 env、`AHFL_SOURCE_DIR` 或 cwd 向上探测 std（2026-07-04）
...
3. **`/lib/std/` legacy tree 与隐式 std 探测已删除**：全仓不再有 tracked `/lib/std/` 源码文件；`project.cpp::builtin_stdlib_search_roots()`、`ProjectInput.stdlib_search_roots`、`ProjectInput.include_stdlib`、`AHFL_SOURCE_DIR` std probe 和 cwd-upward `std/ahfl.toml` probe 已按 RFC 0006 删除。PackageGraph / CLI / LSP 只通过显式 ToolchainProfile/sysroot 建图；raw `ProjectInput` 测试通过显式 module roots 传入 std。
...
9. **B 区三项（try? / JSON-id / transpose-flatten）全部阻断，已逐项核实（2026-06-29）**：
   ...
   - **`Result::transpose`** ✅ 已实现：via 新构造 hook `result_ok/err` + `option_some`（builtin 签名携带类型，绕过 P3 在自由函数体内用 `Result::Ok(v)` 语法构造泛型变体时类型实参退化为 `Any` 的限制）。与 `flatten`（and_then identity）一并完成 M1-3 收口。
