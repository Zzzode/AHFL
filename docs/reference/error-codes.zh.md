...
**触发条件**：整数 range pattern 的下界大于上界。当前 AHFL range pattern 语法为 `-?INT_LITERAL..-?INT_LITERAL`，语义为闭区间匹配。
...
- 负数边界可直接写成 signed range bound，例如 `-3..-1`；这只改变 pattern range bound 语法，不改变表达式中 `-1` 仍为 unary expression 的语义。
