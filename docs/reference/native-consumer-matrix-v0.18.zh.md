...
本文冻结 AHFL V0.18 中 durable-adapter-receipt-persistence-facing consumer 的当前矩阵，重点覆盖 `ExecutionPlan`、`RuntimeSession`、`ExecutionJournal`、`ReplayView`、`SchedulerSnapshot`、`CheckpointRecord`、`CheckpointPersistenceDescriptor`、`PersistenceExportManifest`、`StoreImportDescriptor`、`DurableStoreImportRequest`、`DurableStoreImportDecision`、`DurableStoreImportDecisionReceipt`、`DurableStoreImportDecisionReceiptPersistenceRequest`、`DurableStoreImportDecisionReceiptPersistenceReviewSummary` 各自稳定依赖什么，以及 future real durable store adapter / receipt persistence / recovery explorer 当前应落在哪一层。
...
13. `DurableStoreImportDecisionReceiptPersistenceRequest`
14. `DurableStoreImportDecisionReceiptPersistenceReviewSummary`
...
1. `DurableStoreImportDecisionReceiptPersistenceRequest` 是 future real adapter 当前可稳定消费的 machine-facing persistence request 边界
2. `DurableStoreImportDecisionReceiptPersistenceReviewSummary` 是 projection，不是 durable mutation / recovery ABI
...
1. 跳过 `DurableStoreImportDecisionReceiptPersistenceRequest`，直接在 review / 外部脚本中私造 persistence state machine
2. 把 `DurableStoreImportDecisionReceiptPersistenceReviewSummary` 当 durable receipt persistence 的第一事实来源
3. 跳过 `DurableStoreImportDecisionReceipt`，直接从 request review / decision review / trace 反推 persistence request identity 或 blocker
