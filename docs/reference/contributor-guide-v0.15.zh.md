...
本文给出面向新贡献者的 V0.15 上手路径，重点覆盖 `DurableStoreImportRequest`、`DurableStoreImportReviewSummary`、compatibility contract、consumer matrix、golden regression、CI 标签切片，以及它们与 V0.14 store-import-facing artifact 的协作边界。
...
11. 若扩 reviewer-facing request preview / requested artifact preview / next-step recommendation
   - 最后改 `DurableStoreImportReviewSummary`

这表示：

1. `StoreImportDescriptor` 是 durable-store-import-facing machine state 的直接上游事实来源
2. `DurableStoreImportRequest` 是 durable adapter-facing state 的第一事实来源
3. `DurableStoreImportReviewSummary` 是 request 的 readable projection，不是独立 durable store 状态机
...
5. 允许 future explorer 依赖 `DurableStoreImportRequest` 的 durable request identity、requested artifact set、request boundary、adapter readiness、request status、adapter blocker 与 source version chain
6. 允许 reviewer tooling 依赖 `DurableStoreImportReviewSummary` 的 request preview、requested artifact preview、adapter boundary summary 与 next-step recommendation
...
9. 不允许绕过 `RuntimeSession` / `ExecutionJournal` / `ReplayView` / `SchedulerSnapshot` / `CheckpointRecord` / `CheckpointPersistenceDescriptor` / `PersistenceExportManifest` / `StoreImportDescriptor` / `DurableStoreImportRequest` 直接从 AST、trace、脚本推导 durable store state
...
11. `DurableStoreImportReviewSummary`
...
4. 跳过 `RuntimeSession` / `ExecutionJournal` / `ReplayView` / `SchedulerSnapshot` / `CheckpointRecord` / `CheckpointPersistenceDescriptor` / `PersistenceExportManifest` / `StoreImportDescriptor` / `DurableStoreImportRequest`，直接从 AST、source、trace 重建 durable request identity / requested artifact set / blocker
...
1. 改 `DurableStoreImportRequest` 稳定字段时，要同步更新 durable store import compatibility 文档与 `tests/durable_store_import/*.durable-store-import-request.json`
2. 改 `DurableStoreImportReviewSummary` 稳定字段时，要同步更新 durable store import compatibility 文档与 `tests/durable_store_import/*.durable-store-import-review`
