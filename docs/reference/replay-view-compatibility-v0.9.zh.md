...
1. 当前 replay view 的稳定版本标识是什么，出现在什么位置。
...
- `format_version = "ahfl.replay-view.v2"`
...
1. `ahfl.replay-view.v1`
   - 冻结 V0.8 success-path replay 语义
2. `ahfl.replay-view.v2`
   - 冻结 V0.9 的 failure-aware replay projection

这意味着：

1. `v1` 仍表示 historical success baseline。
2. 需要消费 `runtime_failed` / `partial` replay status、node failed progression 或 workflow failure summary 的 consumer，必须显式接受 `v2`。
3. 不允许把 `v2` 的新 status / consistency 语义伪装成 `v1` 的兼容扩展。

## 当前冻结的兼容层

当前真正冻结的兼容层是：

- deterministic replay state progression 的 failure-aware 投影视图语义边界
...
10. 顶层 `replay_status`
11. 顶层 `workflow_failure_summary`
12. 顶层 `execution_order`
13. `nodes[]`
14. node `node_name`
15. node `target`
16. node `execution_index`
17. node `planned_dependencies`
18. node `satisfied_dependencies`
19. node `saw_node_became_ready`
20. node `saw_node_started`
21. node `saw_node_completed`
22. node `saw_node_failed`
23. node `failure_summary`
24. node `used_mock_selectors`
25. node `final_status`
26. 顶层 `consistency`

当前稳定状态集合包括：

1. `ReplayStatus = Consistent / RuntimeFailed / Partial`
2. node progression `saw_node_failed`
...
5. 未来更细粒度的 host-internal scheduler 状态机
...
4. 顶层 `session_id`
5. 顶层 `replay_status`
6. 顶层 `workflow_failure_summary`
7. 顶层 `execution_order`
8. node progression 四个布尔位
9. node `execution_index`
10. node `failure_summary`
11. node `used_mock_selectors`
12. 顶层 `consistency`

当前 consumer 不应依赖：

1. replay 会承诺真实 runtime recovery ABI
...
4. replay view
   - journal + session 的 failure-aware consistency 投影视图
5. audit report
   - reviewer-facing aggregate artifact

当前契约要求：

1. replay consumer 不应跳过 runtime session / execution journal 自行从 plan 或 trace 重建状态。
2. replay 的 failure 结论必须来自 session / journal 已冻结的稳定边界。
3. audit consumer 若需要 replay 一致性，应优先依赖 replay 的稳定 summary，而不是重新实现 journal -> state projection。
...
4. 为当前 failure-aware 输出补充更明确的文档说明，而不改变旧字段语义
...
1. 删除已有 JSON 字段
2. 重命名已有 JSON 字段
3. 改变 `replay_status`、`workflow_failure_summary`、`execution_order`、`nodes[]` 或 `consistency` 的结构语义
4. 改变 node progression 四个布尔位的语义
5. 让旧 consumer 无法安全忽略新增内容

典型 breaking change 示例：

1. 把 `RuntimeFailed` 从“runtime result failure”改成“任意 replay 构建失败”
2. 把 `Partial` 从“executed prefix 一致”改成“仅表示 workflow 未结束”
3. 把 `saw_node_failed` 删除并改成另一套不兼容事件投影
4. 把 `consistency.journal_matches_execution_order` 从 prefix 语义改成 full-order-only 语义
...
1. 当前稳定版本为 `ahfl.replay-view.v2`
2. `ahfl.replay-view.v1` 继续保留为 success-path historical baseline
3. `emit-replay-view` 已覆盖 success / partial / failed golden
4. replay model / validation / emission 已进入 direct regression 与 CLI regression
