...
- `docs/reference/project-usage.zh.md`
...
```mermaid
flowchart TD
    Descriptor["manifest / workspace"] --> ProjectInput["ProjectInput"]
    ProjectInput --> SourceGraphLoading["source graph loading"]
    SourceGraphLoading --> Pipeline["resolve / typecheck / validate / IR / backend"]
```
...
```mermaid
flowchart TD
    Manifest["project manifest"] --> ProjectInput["ProjectInput { entry_files, search_roots }"]
    ProjectInput --> ParseProject["parse_project(ProjectInput)"]
    ParseProject --> ParseResult["ProjectParseResult { graph, diagnostics }"]
```
