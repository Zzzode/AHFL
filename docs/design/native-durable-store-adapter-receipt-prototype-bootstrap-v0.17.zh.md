...
1. 当前仓库能稳定表达 machine-facing `Receipt`
...
5. 上述能力都建立在 `Decision` 之上，而不是在 review / trace / grep / host log 层私造 receipt state
...
1. adapter-receipt-facing state 的第一落点不是 AST、trace、checkpoint review、persistence review、export review、store import review、durable request review、durable decision review 或 host log，而是正式 `Receipt`
2. `Decision` 仍是 durable store adapter receipt 的直接 machine-facing 上游
...
4. `ReceiptReviewSummary` 仍只是 readable projection，不是 future real durable store adapter / executor / recovery ABI
5. future real durable store adapter / receipt persistence / recovery protocol 只能在 durable receipt compatibility 冻结后继续规划

## Receipt 的第一边界

V0.17 当前建议 `Receipt` 至少回答下面六类问题：
...
1. 不允许先在 review / CLI 层捏造 adapter receipt 语义，再倒逼 `Receipt` 适配
...
1. `Decision` 仍然是 durable adapter receipt 的直接 machine-facing 上游
   - `Receipt` 可以消费 decision
   - 但不应回退让 `DecisionReviewSummary` 变成 machine-facing 第一输入
2. `ReceiptReviewSummary` 仍然只是 projection
...
4. `Decision` 与 `Receipt` 的职责不能合并
   - decision 负责 adapter contract 结论边界
   - receipt 负责 adapter receipt 归档边界

若后续变化触碰下面任一方向，通常就不再只是“补字段”而是分层变化：

1. 让 `DecisionReviewSummary`、`ReviewSummary`、`StoreImportReviewSummary`、`PersistenceExportReviewSummary`、`AuditReport` 或 `DryRunTrace` 反向成为 durable receipt 的第一输入
2. 让 `Receipt` 承担真实 object uploader、database writer、transaction commit protocol 或 recovery launcher ABI
3. 让 `ReceiptReviewSummary` 承担独立 machine-facing taxonomy
...
2. future `Receipt`
   ...
   - 第一输入是 `Decision`
3. future `ReceiptReviewSummary`
   - 回答“当前 reviewer 应如何理解 adapter receipt、blocker 与 next step”
   - 第一输入是 `Receipt`

因此：

1. 不允许用 `DecisionReviewSummary` 倒推 `Receipt`
2. 不允许用 `ReceiptReviewSummary` 倒推真实 durable adapter / receipt persistence / recovery protocol
...
1. future real adapter / recovery protocol 可以复用 `Receipt` 的 receipt identity、receipt outcome、receipt blocker 与 receipt boundary
...
3. 若未来需要把这些 durable 字段提升为稳定输入，应通过新的 compatibility contract 明确演进，而不是静默塞回 `Receipt` / `ReceiptReviewSummary`
