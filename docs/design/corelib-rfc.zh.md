...
- prelude 显式入口；std 通过 `std/ahfl.toml` 作为 sysroot package 分发。
- **验收**：`import std::collections;` 可用；需要 prelude 符号时显式
  `import std::prelude;`。
...
| 5 | prelude stability | prelude 是 lang stability boundary，v1 清单冻结（Option / Result / 容器 / 高阶函数 / 核心 trait / format）；public PackageGraph 入口显式导入；semver：新增 minor、删除/重命名 major + deprecated alias。 | [stdlib-api §9](./corelib-stdlib-api.zh.md) |
...
| 指标 | 数值 | 基准 |
|---|---|---|
| ctest 终态 | 1003 / 1003 | 2026-06-30 Wave-23 Group B-12（P6 prelude module + raw `ProjectInput` prelude_ut 40 assertions）；ctest 1003/1003 全绿；-Werror 0 告警 |
| Build (-Werror -Wswitch) | 163 / 163 targets 零告警 | 同上 |
| stdlib_units 模块数 / 断言数 | **13** / **523** | 12→13（+ prelude_ut 40）；483+40=523（set_ut 84、map_ut 84、prelude_ut 40，以及 option/result/string/list/cmp/time/uuid/json/decimal）；P7 evaluator 另 + 8 assertions； |
...
- **[P6-低 / 便利]** prelude module 与 public 显式入口。✅ **B-12 已完成（2026-06-30）**：`should_inject_prelude()` + `lookup_prelude_symbol()` 两路内部链路打通；`std/prelude.ahfl` 7 类型别名 / 7 构造子 / 38 UFCS 转发器 / 5 容器构造器 全量实装；**prelude_ut 7 家族 40 assertions** 覆盖 raw `ProjectInput` 注入钩子。RFC 0005 后 public PackageGraph 入口统一 `inject_prelude=false`，用户通过显式 `import std::prelude;` 进入 prelude。
...
| **P3 stdlib Priority 2.2–2.9** (Bool/Int/Float/String × 5 traits; Option/Result/List/Set/Map × 容器 traits; json/decimal/uuid/time impl) | 92% | **剩余代码级 blocker：B-2**（2026-07-02 现场确认）：<br> ✅ (a) **已交付**：smoke_p3_impl_{trait,full}.ahfl 本地 impl 删除，旧 stdimpl orphan golden 清理，20 impl 批量入 std/cmp.ahfl + std/fmt.ahfl；COHERENCE_CONFLICT 未触发；ctest 1002/1002<br> (b) impl-body parser 3 语法 gap：`let _ = expr` wildcard 绑定 / `variant_ctor({})` 中 `{}` 在 unit-literal 与 block/empty-struct 间消歧 / `\x: T -> closure_body` 中 closure param name 与 impl-level tparam 作用域冲突<br> ✅ (c) cross-module variant pattern / constructor nullptr 已修复，并由 trait_impl 双回归覆盖。 | **(a)+(c) 已交付；(b) M（约 20 assertions）** |
...
| **P6 std::time / uuid / json / decimal** | 85% | ✅ **Wave-23 Group A 完成**：4 模块 scaffold + UT 全落地；module 级 wrapper 齐全；Decimal 补 max/min/clamp；新增 `time_epoch()` Pure-safe anchor builtin。剩余缺口仅 prelude public package 边界收敛（QoL，非 blocker）。 | **S × 4 已交付**（每模块 ~20 assertions + time_epoch 双注册） |
| **P6 prelude module** | **88%（✅ 完成：B-12 交付）** | `should_inject_prelude()` + `lookup_prelude_symbol()` 双链路作为 raw `ProjectInput` 内部钩子打通；`std/prelude.ahfl` 7 类型别名 / 7 构造子 / 38 UFCS 转发器 / 5 Set·Map 唯一构造器 全量实装；**prelude_ut 7 家族 40 assertions**；防环通过 `!is_std_module()` 门控天然保证。RFC 0005 后 public PackageGraph 入口显式设置 `inject_prelude=false`，用户通过 `import std::prelude;` 进入 prelude。 | **M：38 个 UFCS 转发器 + 7 constructors + 1 UT 文件 40 assertions** |
...
**验收快照（B-1 + B-4 已完，2026-06-30）**：ctest **1002/1002**；P3 完成率 85% → **92%**；Priority 2.2 的 20 impl blocks（Eq/Ord/Hash/Display/Debug × Bool/Int/Float/String）全部落地；COHERENCE_CONFLICT 无回归；smoke_p3_impl_{trait,full} goldens 已重写（删除本地 impl）。
...
**验收快照（B-12 已完，2026-06-30）**：P6 prelude module 全量收口；`std/prelude.ahfl` 7 类型别名 + 7 构造子 + 38 UFCS 转发器 + 5 Set/Map 唯一构造器全部实装；`prelude_ut.ahfl` 40 assertions 覆盖 raw `ProjectInput` 注入钩子；stdlib_units 12→**13 模块**、总断言 483→**523**；P6 完成率 85% → **88%**；ctest 1002 → **1003/1003** 零回归；-Werror 0 告警。

| Step | 对应阶段 | 交付 | 前置 |
|---|---|---|---|
| ✅ **B-1** | P3 | **解 Blocker (a) 已交付**：smoke_p3_impl_{trait,full}.ahfl 重写（删除本地 impl）；旧 orphan single-file stdimpl golden 已清理；COHERENCE_CONFLICT 未触发；std/traits.ahfl 移除未使用的 `import string` | None |
...
| B-12 | P6 | ✅ **已交付** P6 prelude module（2026-06-30，Wave-23 Group B 收尾）。<br>**落地清单**：<br>① `std/prelude.ahfl` §9.1 全清单实装：7 类型别名（Option/Result/Ordering/List/Set/Map+Optional）+ 7 大写构造子（Some/None/Ok/Err/Less/Equal/Greater）+ 小写 `some` + **38 UFCS 转发器**（Option 3、Result 5、collections 21 含 Set/Map 5 个唯一构造器 set_empty/set_singleton/map_empty/map_singleton/map_from_entries、string 3、cmp 7、fmt 1）+ `contains_key<K,V>` 前置；<br>② **内部注入链路**：`project.cpp should_inject_prelude() = inject_prelude && !is_std_module()` 保留为 raw `ProjectInput` 钩子，调用方必须显式传入 std module root；`resolver.cpp lookup_prelude_symbol()` 作为已装载 prelude source unit 的 bare-name fallback；public PackageGraph 入口设置 `inject_prelude=false`，需要用户显式 `import std::prelude;`；<br>③ **防环**：`!is_std_module()` 天然防环（prelude 自身不触发注入）；<br>④ **验收 UT**：`tests/integration/stdlib_units/prelude_ut.ahfl` 7 家族 **40 assertions**（(A) 类型别名 6 / (B) 构造子 6 / (C) Option 5 / (D) Result 4 / (E) List·Set·Map 8 / (F) cmp 6 / (G) string 5）。<br>**验收数字**：ctest 1002→**1003/1003** 零回归；stdlib_units 12→**13 模块**（+prelude_ut）；断言总数 483→**523**（+40）；P6 完成率 85%→**88%**。 | None |
...
- [x] 3 金丝雀：trait_impl Bool-Eq / primitive impl dispatch / g3_repro (MissingSuperTrait)
...
- [x] **[pre-work a]** 退休旧 smoke_p3_impl 本地用户 impl，改为消费 std/traits 批量 impl（否则 coherence conflict）；orphan single-file goldens 已清理
...
##### prelude module 与显式入口（Priority 低 — Wave-23 Group B）✅ 2026-06-30 交付
...
- [x] resolver prelude lookup 链路：`should_inject_prelude()` 门控 + `lookup_prelude_symbol()` bare-name 回退保留为 raw `ProjectInput` 内部钩子；public PackageGraph 入口显式设置 `inject_prelude=false`
- [ ] semver：新增 minor、删除/重命名 major + deprecated alias（PHASE B 配套语法级 `#![no_prelude]` 一起上线）
- [x] 循环导入防环：`!is_std_module()` 门控天然保证（prelude 模块自身不注入）
- [x] 验收：`prelude_ut.ahfl` 7 家族 40 assertions；ctest 1002→1003/1003；stdlib_units 12→13 模块；断言 483→523
