...
本文定义 AHFL V0.18 的 design baseline。V0.17 已经把 `Receipt` / `ReceiptReviewSummary` 做成稳定的 durable-adapter-receipt-facing 输入边界；V0.18 的重点不是直接交付真实 receipt persistence writer、object uploader、database writer、resume token、retry token 或 crash recovery runtime，而是先把“future real durable store adapter 对 receipt 的 machine-facing persistence contract”冻结下来。

换句话说，V0.18 当前要解决的问题是：

1. 当 `Receipt` 已经稳定存在后，future real durable store adapter 最小要生成什么 machine-facing persistence request 才能继续向真实 receipt persistence 执行层演进。
...
1. 当前仓库能稳定表达 machine-facing `PersistenceRequest`
...
5. 上述能力都建立在 `Receipt` 之上，而不是在 review / trace / grep / host log 层私造 persistence state
...
1. adapter-receipt-persistence-facing state 的第一落点不是 AST、trace、checkpoint review、persistence review、export review、store import review、durable request review、durable decision review、durable receipt review 或 host log，而是正式 `PersistenceRequest`
2. `Receipt` 仍是 durable store adapter receipt persistence request 的直接 machine-facing 上游
...
4. `PersistenceReviewSummary` 仍只是 readable projection，不是 future real durable store adapter / receipt persistence / recovery ABI
...
## PersistenceRequest 的第一边界

V0.18 当前建议 `PersistenceRequest` 至少回答下面六类问题：
...
1. 不允许先在 review / CLI 层捏造 receipt persistence 语义，再倒逼 `PersistenceRequest` 适配
...
1. `Receipt` 仍然是 durable adapter receipt persistence 的直接 machine-facing 上游
   - `PersistenceRequest` 可以消费 receipt
   - 但不应回退让 `ReceiptReviewSummary` 变成 machine-facing 第一输入
...
4. `Receipt` 与 `PersistenceRequest` 的职责不能合并
   - receipt 负责 adapter receipt 归档边界
   - persistence request 负责 adapter receipt persistence 归档边界

若后续变化触碰下面任一方向，通常就不再只是“补字段”而是分层变化：

1. 让 `ReceiptReviewSummary`、`DecisionReviewSummary`、`ReviewSummary`、`StoreImportReviewSummary`、`PersistenceExportReviewSummary`、`AuditReport` 或 `DryRunTrace` 反向成为 durable receipt persistence request 的第一输入
2. 让 `PersistenceRequest` 承担真实 object uploader、database writer、transaction commit protocol 或 recovery launcher ABI
...
1. `Receipt`
   ...
   - 第一输入是 `Decision`
2. future `PersistenceRequest`
   - 回答“当前 future adapter contract 对 receipt persistence 的 machine-facing request 到底长什么样”
   - 第一输入是 `Receipt`
3. future `PersistenceReviewSummary`
   - 回答“当前 reviewer 应如何理解 persistence request、blocker 与 next step”
   - 第一输入是 `PersistenceRequest`

因此：

1. 不允许用 `ReceiptReviewSummary` 倒推 `PersistenceRequest`
2. 不允许用 `PersistenceReviewSummary` 倒推真实 durable adapter / receipt persistence / recovery protocol
...
1. future real adapter / recovery protocol 可以复用 `PersistenceRequest` 的 persistence request identity、persistence outcome、persistence blocker 与 persistence boundary
...
3. 若未来需要把这些 durable 字段提升为稳定输入，应通过新的 compatibility contract 明确演进，而不是静默塞回 `PersistenceRequest` / `PersistenceReviewSummary`
