...
1. 当前仓库能稳定表达 machine-facing `Decision`
...
5. 上述能力都建立在 `Request` 之上，而不是在 review / trace / grep / host log 层私造 adapter state
...
1. adapter-decision-facing state 的第一落点不是 AST、trace、checkpoint review、persistence review、export review、store import review、durable request review 或 host log，而是正式 `Decision`
2. `Request` 仍是 durable store adapter decision 的直接 machine-facing 上游
...
4. `DecisionReviewSummary` 仍只是 readable projection，不是 future real durable store adapter / executor ABI
5. future real durable store adapter / receipt / recovery protocol 只能在 durable decision compatibility 冻结后继续规划

## Decision 的第一边界

V0.16 当前建议 `Decision` 至少回答下面六类问题：
...
1. 不允许先在 review / CLI 层捏造 adapter decision 语义，再倒逼 `Decision` 适配
...
1. `Request` 仍然是 durable adapter decision 的直接 machine-facing 上游
   - `Decision` 可以消费 request
   - 但不应回退让 `ReviewSummary` 变成 machine-facing 第一输入
2. `DecisionReviewSummary` 仍然只是 projection
...
4. `Request` 与 `Decision` 的职责不能合并
   - request 负责 adapter-facing request boundary
   - decision 负责 adapter contract 结论边界

若后续变化触碰下面任一方向，通常就不再只是“补字段”而是分层变化：

1. 让 `ReviewSummary`、`StoreImportReviewSummary`、`PersistenceExportReviewSummary`、`AuditReport` 或 `DryRunTrace` 反向成为 durable decision 的第一输入
2. 让 `Decision` 承担真实 object uploader、database writer、transaction commit protocol 或 recovery launcher ABI
3. 让 `DecisionReviewSummary` 承担独立 machine-facing taxonomy
...
2. future `Decision`
   ...
   - 第一输入是 `Request`
3. future `DecisionReviewSummary`
   - 回答“当前 reviewer 应如何理解 adapter decision、capability gap 与 next step”
   - 第一输入是 `Decision`

因此：

1. 不允许用 `ReviewSummary` 倒推 `Decision`
2. 不允许用 `DecisionReviewSummary` 倒推真实 durable adapter / receipt / recovery protocol
...
1. future real adapter / recovery protocol 可以复用 `Decision` 的 decision identity、decision outcome、capability gap 与 decision boundary
...
3. 若未来需要把这些 durable 字段提升为稳定输入，应通过新的 compatibility contract 明确演进，而不是静默塞回 `Decision` / `DecisionReviewSummary`
