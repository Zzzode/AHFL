# AHFL Compiler Architecture

本文给出 AHFL Core 编译器的整体架构说明，面向需要阅读代码、扩展语义阶段或补充 backend 的工程实现者。
...
- [repository-layout.zh.md](./repository-layout.zh.md)
...
include/ahfl/compiler/frontend
include/ahfl/compiler/semantics
include/ahfl/compiler/ir
include/ahfl/compiler/backends

src/compiler/syntax/frontend
...
resolver 不再只处理单个 `ast::Program`，还要显式处理：
...
- `src/compiler/semantics/validate.cpp`
...
不再允许 backend 私下决定 contract/workflow 里的纯布尔 expr 怎么命名。
...
- 显式携带 provenance 和 shared observations。
...
1. [compiler-phase-boundaries.zh.md](./compiler-phase-boundaries.zh.md)
2. 本文
3. [source-graph.zh.md](./source-graph.zh.md)
4. `include/ahfl/compiler/frontend/frontend.hpp`
5. `include/ahfl/compiler/semantics/resolver.hpp`
6. `include/ahfl/compiler/semantics/typecheck.hpp`
7. `include/ahfl/compiler/ir/ir.hpp`
8. [formal-backend.zh.md](./formal-backend.zh.md)
