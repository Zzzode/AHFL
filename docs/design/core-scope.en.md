...
That is directionally correct, but too much for a first language version. A usable first version should solve the first three well enough to support the fourth, while leaving the fifth on a constrained subset.
...
Core should explicitly not optimize for:
...
Core can still reference these concepts through metadata, but should not execute them directly in the language.
...
The `agent` declaration is **declarative only**. It does not contain arbitrary imperative code.
...
It does not contain an implementation body in Core.
...
One of the biggest problems in the current design is that everything is an `Expr`. Core should split expressions into three layers.
...
Core should support **one temporal logic only**. Use LTL first. Do not mix LTL and CTL in the same grammar/version.

Recommended temporal operators:
...
Do not expose raw `A` and `E` in Core.
...
Unicode operators can be formatter sugar later, but should not be part of the canonical grammar.

## Core Type System

Core should prefer a small, explicit, largely nominal type system.

### Primitive Types

Source-level primitive types in Core are:
...
These are compiler-internal concepts. They are not part of the AHFL Core source syntax and
must not appear in user-written programs.

### Type Policy

Core should use these rules:
...
### Phase 2 (Lifecycle & Observability)
...
AHFL Core should be a small, declarative, statically checked control language for agent boundaries and workflow policy.

If this core design succeeds at that, the native fusion story can be added later without collapsing the language under its own weight.
