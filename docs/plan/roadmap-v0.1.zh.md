# AHFL Core V0.1 Roadmap Baseline

本文保留为 AHFL Core V0.1 的历史路线图基线，用于解释当前 V0.2 设计是从哪条已完成路径演进而来；它不是当前版本的执行入口。
...
3. `docs/design/compiler-phase-boundaries-v0.2.zh.md` 冻结 `parse -> ast -> resolve -> typecheck -> validate -> emit` 阶段边界
...
1. `ahflc emit-smv`
2. `docs/design/formal-backend-v0.2.zh.md`
...
本文对应的 V0.1 规划项已经全部完成，后续工作不再属于本轮 roadmap 里的 issue-ready 范围。
