...
55. Leaf-pattern replacement completion 已落库：LSP pattern completion 现在也会在 source-safe leaf pattern token 上发出 `CompletionItem.textEdit`，覆盖 literal pattern、signed Int range pattern 和 unit enum variant pattern；例如 `Some(1)`、`Some(0..1)` 和 `Data { label: Low }` 会替换当前 leaf token，而不是在 token 旁边裸插入候选文本。tuple 和 or-pattern 不自动替换，避免误改复合 pattern 结构。
...
3. Release evidence includes representative finite/open/nested pattern cases. 状态：`rfc0011.pattern_matrix.representative_cases` 已进入仓库级 release evidence archive，覆盖 finite enum、nested enum payload、open Int default arm、bounded Int range 和 `String(0,0)` singleton pattern 的真实 package check。
...
- 2026-07-09: Added leaf-pattern replacement completion. Pattern completions now replace source-safe literal, signed Int range and unit enum variant pattern tokens via `CompletionItem.textEdit` while leaving tuple and or-pattern structures manual.
- 2026-07-09: Extended nested enum payload snippet choices to payload variant skeletons. Snippet placeholders now offer tuple and struct enum payload shapes such as `Wrap(_)` and `Tagged { code: _ }`, with snippet-choice escaping for skeletons containing commas.
