...
33. Bounded Int arithmetic range inference 首个切片已落库：一元 `-` 作用于 bounded Int operand 时推导 `Int(-max, -min)`；二元 `+`、`-`、`*` 和静态排除零除数的 `/` 在两个 operand 都是 `BoundedInt` 时会推导闭区间结果；`%` 在 divisor magnitude interval 可物化时给出精确 remainder 闭区间，对过大的 non-singleton divisor magnitude interval 给出保守 remainder 闭区间；expected bounded Int 会向 arithmetic operand 传递 literal singleton hint，因此 `let x: Int(0, 5) = 1 + 2` 可通过、`let x: Int(0, 2) = 1 + 2` 会由既有 subtype relation 拒绝；溢出、除数区间可能包含 0 或触发 `int64` `min / -1` 边界时保守回退普通 `Int`，不猜测错误区间。
...
2. range pattern v1 仍只覆盖 signed integer literal 闭区间；非 literal refinement propagation 已有 bounded operand `+` / `-` / `*`、非零 `/` 和 finite variable-divisor 精确 / large-domain 保守 `%` 首个切片，Decimal multiplication product-scale semantics 与显式 Decimal division target-scale / rounding API 已落库，但超大 variable-divisor 精确 modulo interval 和未来 Float refinement semantics 仍未稳定。
...
- 2026-07-09: Added exact bounded Int modulo hulls for materialized finite divisor magnitude intervals, covering positive, negative, mixed-sign lhs intervals and negative divisor ranges; oversized divisor domains still use the conservative hull.
