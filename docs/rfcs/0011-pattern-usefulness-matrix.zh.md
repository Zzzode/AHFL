...
2. range pattern v1 仍只覆盖 signed integer literal 闭区间；非 literal refinement propagation 已有 bounded operand `+` / `-` / `*`、非零 `/` 和 finite variable-divisor 精确 / large-domain 保守 `%` 首个切片，Decimal multiplication product-scale semantics 已落库，但超大 variable-divisor 精确 modulo interval、Decimal division rounding / target-scale policy 和未来 Float refinement semantics 仍未稳定。
