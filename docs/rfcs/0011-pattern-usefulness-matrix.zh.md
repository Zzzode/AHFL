...
33. Bounded Int arithmetic range inference 首个切片已落库：一元 `-` 作用于 bounded Int operand 时推导 `Int(-max, -min)`；二元 `+`、`-`、`*` 和静态排除零除数的 `/` 在两个 operand 都是 `BoundedInt` 时会推导闭区间结果；`%` 在 divisor 为 singleton bounded interval 时给出精确 remainder 闭区间，对非 singleton divisor 给出保守 remainder 闭区间；expected bounded Int 会向 arithmetic operand 传递 literal singleton hint，因此 `let x: Int(0, 5) = 1 + 2` 可通过、`let x: Int(0, 2) = 1 + 2` 会由既有 subtype relation 拒绝；溢出、除数区间可能包含 0 或触发 `int64` `min / -1` 边界时保守回退普通 `Int`，不猜测错误区间。
