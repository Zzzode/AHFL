...
2. range pattern v1 仍只覆盖 signed integer literal 闭区间；非 literal refinement propagation 已有 bounded operand `+` / `-` / `*`、非零 `/`、finite variable-divisor 精确 `%` 和 divisor-dominates oversized exact `%` 首个切片，Decimal multiplication product-scale semantics 与显式 Decimal division target-scale / rounding API 已落库，但 general large-domain variable-divisor exact modulo hull 和未来 Float refinement semantics 仍未稳定。
