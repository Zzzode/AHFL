...
- [frontend-architecture-v0.1.md](/Users/bytedance/Develop/AHFL/docs/frontend-architecture-v0.1.md)
...
3. `docs/frontend-architecture-v0.1.md` 冻结 `parse -> ast -> resolve -> typecheck -> validate -> emit` 阶段边界
...
1. 设计下一个后端边界，例如模型检查器或其他执行后端的 lowering
2. 增加更多复杂 workflow / temporal / IR golden 用例
3. 将稳定 IR 与 backend-specific lowering API 明确拆层，并保持 JSON 输出可供外部工具直接消费
